...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    # find lid_concept
    lid_concept = find_concept_by_template_name("lid")
    body_concept = find_concept_by_template_name("body")
    if lid_concept is None or body_concept is None:
        raise GenGifError("Lid or body concept not found in conceptualization.")
    axis_pos = np.array(lid_concept["parameters"]["position"])
    lid_height = lid_concept["parameters"]["outer_size"][2]

    body_params = body_concept["parameters"]
    body_top_height = body_params["position"][1]
    for i in range(int(body_params["num_levels"][0])):
        body_top_height += body_params[f"level_{i + 1}_size"][1]

    body_top_height = min(body_top_height, axis_pos[1])  # ensure it does not exceed lid height
    move_state_and_limits = []
    lid_state_and_limits = np.array(
        [body_top_height, axis_pos[1] + lid_height, axis_pos[1]]
    )
    move_state_and_limits.append(lid_state_and_limits)
    ...
    actionable_parts_info["move_axises"] = [[0, 1, 0]]  # vertical axis, upward

...
def generate_grasp_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, part_names=["cover"]):
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Box", manipulation_knowledge)
    )
    ...
    find_fourfold_cover = False
    concept_template_names = [concept["template"] for concept in concepts]
    find_fourfold_cover = (
        find_text_in_list("Fourfold_Cover", concept_template_names) != -1
    )
    ...
    cur_gripper_param = -1
    for i in range(gripper_frames_num):
        gripper_params[i].append(cur_gripper_param)
        cur_gripper_param += per_frame_delta
    ...
    if find_fourfold_cover:
        ...
        for i in range(3 * num_frames):
            if i < 2 * num_frames:
                ...
                for mesh in part_meshes.values():
                    vis.add_geometry(mesh)
                grasp_pose, force_direction, gripper_width_ratio, _ = (
                    get_manip_knowledge(
                        concepts,
                        "Box",
                ...
                for semantic, mesh in part_meshes.items():
                    ...
                    if template_idx == -1:
                        continue
                    direction = (
                        1 if (i - 2 * num_frames) < turn_times[template_idx] else -1
                    )
...
def generate_affordance_knowledge_gif(
    ...
    if find_fourfold_cover:
        # First open all, then close all
        for concept in concepts:
            if concept["template"] == "Fourfold_Cover":
                fourfold_cover_concept = concept
                break

        cover_rotations = fourfold_cover_concept["parameters"]["cover_rotation"]
        to_max_angles = [0, 0, 0, 0]
        total_angle = 0
        total_time = num_frames * 4
        for idx, cover_rotation in enumerate(cover_rotations):
            if idx in [0, 2]:  # -90->180
                min_angle, max_angle = -90, 150  # degree
                to_max_angles[idx] = max_angle - cover_rotation
                total_angle += (max_angle - cover_rotation) + (max_angle - min_angle)
            elif idx in [1, 3]:  # 90 ->-180
                min_angle, max_angle = 90, -150
                to_max_angles[idx] = cover_rotation - max_angle
                total_angle -= (max_angle - cover_rotation) + (max_angle - min_angle)
            else:
                raise ValueError("Invalid index for cover rotation.")
        turn_angles = np.sum(to_max_angles)  # compute the total angle to turn down
        sorted_angle_idx = np.argsort(to_max_angles)  # sort the angles to turn down from small to large
        turn_time = int(np.ceil(total_time * turn_angles / total_angle))
        angle_per_frame = total_angle / total_time
        angle_per_frames = np.zeros(4)
        for idx in range(4):
            if idx in [0, 2]:
                angle_per_frames[idx] = angle_per_frame
            else:
                angle_per_frames[idx] = -angle_per_frame

        sorted_cover_angles = np.zeros(8)
        cur_angle = 0
        for idx, sorted_idx in enumerate(sorted_angle_idx):
            sorted_cover_angles[idx] = cur_angle + to_max_angles[sorted_idx]
            cur_angle = sorted_cover_angles[idx]
        assert abs(cur_angle - turn_angles) < 1, f"{cur_angle} != {turn_angles}"
        remain_angle = total_angle - turn_angles
        for idx in range(len(sorted_angle_idx)):
            sorted_cover_angles[4 + idx] = cur_angle + remain_angle / 4
            cur_angle = sorted_cover_angles[4 + idx]
        assert abs(cur_angle - total_angle) < 1, f"{cur_angle} != {total_angle}"
        sorted_cover_times = np.ceil(total_time * sorted_cover_angles / total_angle)

        for i in range(total_time):
            vis.clear_geometries()
            # Direction Control: First Half Forward,Second Half Backward
            if i < turn_time:
                if i <= sorted_cover_times[0]:
                    idx = sorted_angle_idx[0]
                elif i <= sorted_cover_times[1]:
                    idx = sorted_angle_idx[1]
                elif i <= sorted_cover_times[2]:
                    idx = sorted_angle_idx[2]
                else:
                    idx = sorted_angle_idx[3]
                direction = 1
                fourfold_cover_concept["parameters"]["cover_rotation"][idx] += (
                    direction * angle_per_frames[idx]
                )
            else:
                if i <= sorted_cover_times[4]:
                    idx = sorted_angle_idx[0]
                elif i <= sorted_cover_times[5]:
                    idx = sorted_angle_idx[1]
                elif i <= sorted_cover_times[6]:
                    idx = sorted_angle_idx[2]
                else:
                    idx = sorted_angle_idx[3]
                direction = -1
        ...
        for move_axis, move_state_and_limit in zip(move_axises, move_state_and_limits):
            ...
            turn_times.append(
                int(
                    np.ceil(
                        num_frames * (max_angle - rotation_angle) / total_angle_range
                    )
                )
            )
            angle_per_frame = total_angle_range / num_frames

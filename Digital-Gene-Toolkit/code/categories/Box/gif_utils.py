...
def assemble_concepts_instance(concepts):
    ...
    for c in concepts:
        ...
        try:
            ...
            if module_name == "Fourfold_Cover":
                num_vertices = len(component.vertices)
                num_faces = len(component.faces)
                has_cover = component.has_cover
                num_cover = np.sum(has_cover)
                per_cover_faces = num_faces // num_cover
                per_cover_vertices = num_vertices // num_cover
                assert per_cover_faces == 12, f"per_cover_faces: {per_cover_faces}"
                assert per_cover_vertices == 8, (
                    f"per_cover_vertices: {per_cover_vertices}"
                )
                for cover_idx in range(num_cover):
                    instance_name = f"{component.semantic}_{cover_idx}"
                    ...
                    vertices_list[instance_name].append(
                        component.vertices[
                            cover_idx * per_cover_vertices : (cover_idx + 1)
                            * per_cover_vertices
                        ]
                    )
                    faces_list[instance_name].append(
                        component.faces[
                            cover_idx * per_cover_faces : (cover_idx + 1)
                            * per_cover_faces
                        ]
                        - cover_idx * per_cover_vertices
                    )
                    total_num_vertices[instance_name] += cover_idx * per_cover_vertices
            else:
                vertices_list.setdefault(component.semantic, [])
                faces_list.setdefault(component.semantic, [])
                total_num_vertices.setdefault(component.semantic, 0)
...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    rot_rad = np.radians(np.array(rotation_angle))
    rotation_axis = idx2axis[rotation_axis_idx]
    assert rotation_axis_idx == 0, "Box cover rotation should be along x axis only."
    assert rotation_angle <= 0, (
        "Box cover rotation should be along x axis and postive angles."
    )
    # only one case of rotation, positive rotation along x-axis
    if rotation_axis_idx == 0:
        rot_offset = np.array([0, 0, 0])
    else:
        print(
            f"Box Regular cover rotation_axis_idx: {rotation_axis_idx}, rotation_angle: {rotation_angle}"
        )
        raise ValueError(
            "cover rotation axis should be along x axis and postive angles."
        )
    axis_pos += rot_offset

    move_state_and_limits = np.zeros((1, 3))
    move_state_and_limits[:] = np.array([-120, 0, rotation_angle])

    actionable_parts_info["template_names"] = [cover_concept["template"]]

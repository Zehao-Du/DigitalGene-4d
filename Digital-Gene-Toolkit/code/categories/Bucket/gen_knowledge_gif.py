...
def generate_grasp_knowledge_gif(
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Bucket", manipulation_knowledge)
    )
    ...
    move_steps = []
    turn_times = []
    for move_type, move_axis, move_state_and_limit in zip(
        move_types, move_axises, move_state_and_limits
    ):
        if move_type == "revolute":
        ...
        elif move_type == "prismatic":
            min_dist, max_dist, translate_dist = move_state_and_limit
            total_translate_range = (max_dist - translate_dist) + (max_dist - min_dist)
            turn_times.append(
                int(
                    np.ceil(
                        num_frames * (max_dist - translate_dist) / total_translate_range
                    )
                )
            )
            translation_per_frame = total_translate_range / num_frames
            move_axis = move_axis / np.linalg.norm(move_axis)
            move_steps.append(translation_per_frame * move_axis)
        else:
    ...
    for i in range(3 * num_frames):
        if i < 2 * num_frames:
            ...
            for semantic, mesh in part_meshes.items():
                ...
                if template_idx == -1:
                    continue
                direction = 1 if (i - 2 * num_frames) < turn_times[template_idx] else -1

                if move_types[template_idx] == "revolute":
                ...
                elif move_types[template_idx] == "prismatic":
                    translation = move_steps[template_idx] * direction
                    mesh.translate(translation, relative=True)
                    gripper_mesh.translate(translation, relative=True)
                else:
                    raise ValueError(
                        f"Unsupported move type: {move_types[template_idx]}"
                    )
                vis.update_geometry(mesh)
    ...
    for i in range(num_frames):
        for semantic, mesh in part_meshes.items():
            ...
            if move_types[template_idx] == "revolute":
                ...
                raise ValueError(f"Unsupported move type: {move_types[template_idx]}")
            vis.update_geometry(mesh)

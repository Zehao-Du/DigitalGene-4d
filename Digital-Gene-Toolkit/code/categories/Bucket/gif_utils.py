...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    # find handle_concept and handle_concept
    handle_concept = find_concept_by_template_name("handle")
    cylinder_concept = find_concept_by_template_name("cylinder")
    handle_concept_rotation = np.array(handle_concept["parameters"]["rotation"])
    handle_concept_rotation_rad = np.radians(handle_concept_rotation)
    handle_concept_pos = np.array(handle_concept["parameters"]["position"])

    if handle_concept["template"] in [
        "Round_U_Handle",
        "Flat_U_Handle",
        "Trifold_Handle",
    ]:
        rot_axis = np.array([1, 0, 0])
        rot_axis = apply_transformation(
            rot_axis, [0, 0, 0], handle_concept_rotation_rad
        )
        handle_rotation = handle_concept_rotation[0]
        handle_axis_pos = handle_concept_pos

    elif handle_concept["template"] in ["Curved_Handle"]:
        radius = handle_concept["parameters"]["radius"]
        exist_angle = handle_concept["parameters"]["exist_angle"][0]
        exist_angle = np.radians(exist_angle)
        rot_axis = np.array([1 - np.cos(exist_angle), -np.sin(exist_angle), 0])
        rot_axis = rot_axis / np.linalg.norm(rot_axis)
        rot_axis = apply_transformation(
            rot_axis, [0, 0, 0], handle_concept_rotation_rad, rotation_order="ZXY"
        )
        handle_rotation = handle_concept_rotation[0]  # degree
        handle_axis_pos = np.array([radius[0], 0, 0])
        handle_axis_pos = apply_transformation(
            handle_axis_pos,
            handle_concept_pos,
            handle_concept_rotation_rad,
            rotation_order="ZXY",
        )

    else:
        raise ValueError(
            "Unknown handle template: {}".format(handle_concept["template"])
        )

    move_state_and_limits = []
    handle_rotation = np.clip(
        handle_rotation, -130, 130
    )  # limit handle rotation to [-130, 130] degrees
    handle_state_and_limit = np.array([-130, 130, handle_rotation])
    move_state_and_limits.append(handle_state_and_limit)

    actionable_parts_info["template_names"] = [handle_concept["template"]]
    actionable_parts_info["move_types"] = ["revolute"]
    actionable_parts_info["move_centers"] = [handle_axis_pos]
    actionable_parts_info["move_axises"] = [rot_axis]

    if cylinder_concept is not None:
        move_axis_pos = cylinder_concept["parameters"]["position"]
        move_axis_pos = np.array(move_axis_pos)
        cylinder_size = cylinder_concept["parameters"]["size"]
        cylinder_height = cylinder_size[1]
        bucket_top = move_axis_pos[1]  # assume the initial lid is just above the bucket

        actionable_parts_info["template_names"] += [cylinder_concept["template"]]
        actionable_parts_info["move_types"] += ["prismatic"]
        actionable_parts_info["move_centers"] += [move_axis_pos]
        actionable_parts_info["move_axises"] += [np.array([0, 1, 0])]  # up direction

        move_state_and_limits.append(
            np.array([bucket_top, bucket_top + cylinder_height * 3, bucket_top])
        )

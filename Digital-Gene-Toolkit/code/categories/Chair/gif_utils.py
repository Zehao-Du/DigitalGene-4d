...
def assemble_concepts_instance(concepts):
    ...
    for c in concepts:
        ...
        try:
            ...
            if module_name in ["Regular_leg_with_splat", "Office_armrest"]:
                if module_name == "Regular_leg_with_splat":
                ...
                elif module_name == "Office_armrest":
                    per_part_vertices = [0, 8, 8, 8, 8]
                    per_part_faces = [0, 12, 12, 12, 12]
                    cum_part_vertices = np.cumsum(per_part_vertices, dtype=int)
                    cum_part_faces = np.cumsum(per_part_faces, dtype=int)
                    part2instance = [0, 1, 0, 1]
                ...
                if module_name == "Solid_armrest":
                    num_instance = 2
                elif module_name == "Regular_leg":
                    number_of_legs = component.number_of_legs
                    num_instance = number_of_legs[0]
                else:
                    num_instance = 1

                per_instance_vertices = len(component.vertices) // num_instance
                per_instance_faces = len(component.faces) // num_instance
                assert per_instance_vertices * num_instance == len(
                    component.vertices
                ), (
                    f"vertices number for every instance not matching: {per_instance_vertices} * {num_instance} != {len(component.vertices)}"
                )
...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    """
    raise GenGifError(
        "analyze_actionable_parts_from_conceptualization is not implemented for chair yet."
    )
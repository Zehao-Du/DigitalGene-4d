...
class Regular_lever(ConceptTemplate):
    def __init__(
        ...
        level_support_1_position = [
            -level_support_seperation[0] - level_support_size[0] / 2,
            0,
            0,
        ]
        self.level_support_mesh_1 = Cuboid(
            level_support_size[1],
            level_support_size[0],
            level_support_size[2],
        ...
        level_support_2_position = [
            level_support_seperation[0] + level_support_size[0] / 2,
            0,
            0,
        ]
        self.level_support_mesh_2 = Cuboid(
            level_support_size[1],
            level_support_size[0],
            level_support_size[2],
        ...
        level_handle_3_position = [
            0,
            level_handle_offset[0] * np.cos(level_handle_rotation[0]),
            (level_support_size[2] + level_handle_size[2]) / 2
            - level_handle_offset[0] * np.sin(level_handle_rotation[0]),
        ]
        self.level_handle_mesh_3 = Cuboid(
            level_handle_size[1],
            level_handle_size[0],
            level_handle_size[2],
        ...
        level_handle_4_position = [
            0,
            level_handle_offset[0] * np.cos(level_handle_rotation[0]),
            -(level_support_size[2] + level_handle_size[2]) / 2
            + level_handle_offset[0] * np.sin(level_handle_rotation[0]),
        ]
        self.level_handle_mesh_4 = Cuboid(
            level_handle_size[1],
            level_handle_size[0],
            level_handle_size[2],
...
class Regular_jaw(ConceptTemplate):
    def __init__(
        self, size, jaw_separation, jaw_rotation, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
...
class Curved_jaw(ConceptTemplate):
    def __init__(
        ...
        self.jaw_mesh_1 = Ring(
            size[2],
            size[0],
            size[1],
            exist_angle=central_angle[0],
            rotation=jaw_mesh_1_pre_rotation,
        )
        ...
        self.jaw_mesh_1.vertices = (
            self.jaw_mesh_1.vertices - jaw_mesh_1_position
        ) @ jaw_mesh_1_rotation_matrix.T + jaw_mesh_1_position
        self.jaw_mesh_1.vertices = apply_transformation(
            self.jaw_mesh_1.vertices,
            [0, 0, (size[0] + size[1]) / 2 - jaw_separation[0]],
            [0, 0, 0],
        )

...
def generate_affordance_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, part_names=["lever"]):
    ...
    for concept in concepts:
        if "lever" in concept["template"].lower():
            lever_concept = concept
        elif "jaw" in concept["template"].lower():
            jaw_concept = concept
    assert lever_concept is not None, "No standard lever found in the list"

    lever_rotation = abs(lever_concept["parameters"]["level_handle_rotation"][0])
    lever_rotation = max(lever_rotation, 0.01)  # ensure non-zero rotation
    min_angle, max_angle = -lever_rotation, lever_rotation
    total_angle_range = max_angle - min_angle
    turn_times.append(int(np.ceil(num_frames * lever_rotation / total_angle_range)))
    ...
    for i in range(num_frames):
        ...
        for idx in range(len(turn_times)):
            ...
            lever_concept["parameters"]["level_handle_rotation"][0] += (
                direction * angle_per_frame
            )
            if jaw_concept != None:
                jaw_concept["parameters"]["jaw_rotation"][0] += (
                    direction * angle_per_frame
                )

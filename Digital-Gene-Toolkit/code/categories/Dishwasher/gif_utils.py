...
def assemble_concepts_instance(concepts):
    ...
    for c in concepts:
        ...
        try:
            ...
            if "button" in module_name.lower():
                num_button = int(component.num_buttons[0] + 1)
                per_button_vertices = len(component.vertices) // num_button
                per_button_faces = len(component.faces) // num_button
                assert per_button_vertices * num_button == len(component.vertices), (
                    f"The number of vertices does not match for each button: {per_button_vertices} * {num_button} != {len(component.vertices)}"
                )
                for button_idx in range(num_button):
                    instance_name = f"{component.semantic}_{button_idx}"
                    ...
                    vertices_list[instance_name].append(
                        component.vertices[
                            button_idx * per_button_vertices : (button_idx + 1)
                            * per_button_vertices
                        ]
                    )
                    faces_list[instance_name].append(
                        component.faces[
                            button_idx * per_button_faces : (button_idx + 1)
                            * per_button_faces
                        ]
                        - button_idx * per_button_vertices
                    )
...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    assert rotation_axis_idx != 2, "Door rotation should not be along z axis."
    # rotation can be either x-axis (up/down) or y-axis (left/right)
    if rotation_axis_idx == 0:
        if rotation_angle < 0:  # x-axis down
            rot_offset = np.array(
                [
                    0,
                    np.cos(rotation_angle_rad)
                    * door_concept["parameters"]["size"][1]
                    / 2,
    ...
    actionable_parts_info["template_names"] = [door_concept["template"]]
    concept_num = 1
    if handle_concept is not None:
        actionable_parts_info["template_names"].append(handle_concept["template"])
        concept_num = 2

    move_state_and_limits = np.zeros((concept_num, 3))
    if rotation_angle > 0:

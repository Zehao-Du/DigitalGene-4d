...
class Trifold_Handle_Knowledge(KnowledgeWrapper):
    ...
    def __init__(self, concept, category, manipulation_knowledge, *manip_args):
        ...
        if obj.grip_size[2] + obj.mounting_size[2] < self.finger_length:
            self.grasp_position[2] += (
                self.finger_length - obj.grip_size[2] - obj.mounting_size[2]
            )
...
class Cuboidal_Handle_Knowledge(KnowledgeWrapper):
    ...
    def __init__(self, concept, category, manipulation_knowledge, *manip_args):
        ...
        if size[1] > size[0]:
            self.grasp_position = np.array(
                [0, up_ratio * size[1] * 0.25, obj.size[2] / 2 + self.gripper_length]
            )

            self.grasp_rotation = np.array([[0, 1, 0], [1, 0, 0], [0, 0, -1]])
        else:
            self.grasp_position = np.array(
                [up_ratio * size[0] * 0.25, 0, obj.size[2] / 2 + self.gripper_length]
            )
            self.grasp_rotation = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, -1]])

        if self.finger_length > size[2]:
            self.grasp_position[2] += self.finger_length - size[2]

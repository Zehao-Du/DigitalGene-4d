...
class Multilevel_Body(ConceptTemplate):
    def __init__(
        ...
        for i in range(round(num_levels[0]) - 1):
            delta_height += locals()["level_" + str(i + 2) + "_size"][1] / 2
            mesh_position = [0, delta_height, 0]
            delta_height += locals()["level_" + str(i + 2) + "_size"][1] / 2
            self.mesh = Cylinder(
                locals()["level_" + str(i + 2) + "_size"][1],
                locals()["level_" + str(i + 2) + "_size"][0],
        ...
        self.vertices = apply_transformation(
            self.vertices, position, rotation, rotation_order="YXZ"
        )
...
class Press_Nozzle(ConceptTemplate):
    def __init__(
        ...
        total_offset_z = (
            ...
            + locals()["level_%d_size" % (num_levels[0])][0]
        )
        ...
        if num_nozzles[0] == 2:
            ...
            nozzle_mesh_position = [
                ...
                delta_height
                + nozzle_offset[0]
                - nozzle_length[0] * np.sin(nozzle_rotation[0])
...
class Spray_Nozzle(ConceptTemplate):
    def __init__(
        ...
        bottom_mesh_position = [0, delta_height, 0]
        self.bottom_mesh = Cylinder(
            bottom_size[1], bottom_size[0], position=bottom_mesh_position
        )
        vertices_list.append(self.bottom_mesh.vertices)
        faces_list.append(self.bottom_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.bottom_mesh.vertices)
        ...
        middle_mesh_position = [0, delta_height, 0]
        ...
        nozzle_mesh_position = [
            ...
            delta_height
            + top_offset_y
            - nozzle_total_offset_z * np.sin(top_rotation[0]),
        ...
        handle_total_offset_z = (
            ...
            + handle_offset[0]
            - handle_size[2] / 2
            - handle_size[1] / 2 * np.sin(handle_rotation[0])
        )
        handle_total_offset_y = (
            -handle_size[1] / 2 * np.cos(handle_rotation[0]) - top_size[1] / 2
        )
        handle_mesh_position_1 = [0, handle_total_offset_y, handle_total_offset_z]
        handle_mesh_rotation = [top_rotation[0], 0, 0]
        handle_mesh_position_1 = adjust_position_from_rotation(
            handle_mesh_position_1, handle_mesh_rotation
        )

        handle_mesh_position_2 = [0, delta_height + top_offset_y, 0]
        handle_mesh_position = list_add(handle_mesh_position_1, handle_mesh_position_2)
        handle_mesh_rotation[0] += handle_rotation[0]

        self.handle_mesh = Cuboid(
            handle_size[1],
            handle_size[0],
            handle_size[2],

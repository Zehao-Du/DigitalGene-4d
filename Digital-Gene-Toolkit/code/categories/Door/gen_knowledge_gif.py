...
def generate_grasp_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, part_names=["standard_door"]):
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Door", manipulation_knowledge)
    )
    ...
    for concept in concepts:
        if concept["template"] == "Standard_Door":
            door_concept = concept
            break
    assert door_concept is not None, "No Standard_Door concept found in the list."
    ...
    existence_of_door = door_concept["parameters"]["existence_of_door"]
    if handle_concept is not None:
        existence_of_handle = handle_concept["parameters"]["existence_of_handle"]
    for door in [0, 1]:
    ...
    if handle_concept is not None:
        if handle == 0 and handle_concept["template"] in [
            "Cylindrical_Handle",
            "LShape_Handle",
            "Spherical_Handle",
        ]:
            # Get current camera parameters (pinhole camera model)
            current_camera_params = vis.get_view_control().convert_to_pinhole_camera_parameters()
            # Extract rotation matrix R and translation vector t from the extrinsic matrix
            R = current_camera_params.extrinsic[:3, :3]
            t = current_camera_params.extrinsic[:3, 3]
            # Compute the camera position in world coordinates: camera_pos = -R^T @ t
            camera_pos = -R.T @ t
            # Calculate the forward direction vector of the camera (negative Z-axis in Open3D)
            forward_vector = -R.T @ np.array([0, 0, 1])
            # Compute a look-at point located in front of the camera
            lookat_point = camera_pos + forward_vector * 10.0  # 10.0 is an arbitrary distance factor
            # Flip the camera position across the Y axis (equivalent to rotating 180 degrees around Y)
            new_camera_pos = np.array([-camera_pos[0], camera_pos[1], -camera_pos[2]])
            # Define the up vector (keep Y as the upward direction)
            up_vector = np.array([0.0, 1.0, 0.0])
            # Recompute the extrinsic matrix with the new camera position, looking at the origin
            new_extrinsic = compute_lookat_open3d(new_camera_pos, np.array([0, 0, 0]), up_vector)
            # Update the extrinsic parameters in the camera parameters
            current_camera_params.extrinsic = new_extrinsic
            # Apply the updated camera parameters to the visualization view
            vis.get_view_control().convert_from_pinhole_camera_parameters(current_camera_params)
    ...
    door_rotations = door_concept["parameters"]["door_rotation"]
    if handle_concept is not None:
        handle_door_rotations = handle_concept["parameters"]["door_rotation"]
        assert np.allclose(door_rotations, handle_door_rotations), (
            "Handle and door rotation angles must be the same."
        )
    min_angle, max_angle = -90, 90  # degree
    for door_rotation in door_rotations:
        total_angle_range = (max_angle - door_rotation) + (max_angle - min_angle)
        turn_times.append(
            int(np.ceil(num_frames * (max_angle - door_rotation) / total_angle_range))
        )
    ...
    for i in range(3 * num_frames):
        if i < 2 * num_frames:
            ...
            for idx in range(len(turn_times)):
                ...
                door_concept["parameters"]["door_rotation"][idx] += (
                    direction * angle_per_frame
                )
                if handle_concept is not None:
                    handle_concept["parameters"]["door_rotation"][idx] += (
                        direction * angle_per_frame
                    )
            ...
            grasp_pose, force_direction, gripper_width_ratio, _ = get_manip_knowledge(
                concepts, "Door", manipulation_knowledge
            )
            gripper_mesh = deepcopy(origin_gripper_mesh).transform(grasp_pose)
            vis.add_geometry(gripper_mesh)
            ctr = vis.get_view_control()

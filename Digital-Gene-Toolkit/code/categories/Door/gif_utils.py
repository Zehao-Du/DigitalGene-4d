...
def assemble_concepts_instance(concepts):
    ...
    for c in concepts:
        ...
        try:
            ...
            if module_name == "Standard_Door":
                existence_of_door = component.existence_of_door
                num_door = int(np.sum(existence_of_door))
                per_door_vertices = len(component.vertices) // num_door
                per_door_faces = len(component.faces) // num_door
                for door_idx in range(num_door):
                    instance_name = f"{component.semantic}_{door_idx}"
                    ...
                    vertices_list[instance_name].append(
                        component.vertices[
                            door_idx * per_door_vertices : (door_idx + 1)
                            * per_door_vertices
                        ]
                    )
                    faces_list[instance_name].append(
                        component.faces[
                            door_idx * per_door_faces : (door_idx + 1) * per_door_faces
                        ]
                        - door_idx * per_door_vertices
                    )
                    total_num_vertices[instance_name] += door_idx * per_door_vertices
            elif "handle" in module_name.lower():
                existence_of_door = component.existence_of_door
                existence_of_handle = component.existence_of_handle
                num_door = int(np.sum(existence_of_door))
                num_handle = int(np.sum(existence_of_handle) * num_door)
                per_handle_vertices = len(component.vertices) // num_handle
                per_handle_faces = len(component.faces) // num_handle
                for handle_idx in range(num_handle):
                    instance_name = f"{component.semantic}_{handle_idx}"
                    vertices_list.setdefault(instance_name, [])
                    faces_list.setdefault(instance_name, [])
                    total_num_vertices.setdefault(instance_name, 0)
                    ...
                    vertices_list[instance_name].append(
                        component.vertices[
                            handle_idx * per_handle_vertices : (handle_idx + 1)
                            * per_handle_vertices
                        ]
                    )
                    faces_list[instance_name].append(
                        component.faces[
                            handle_idx * per_handle_faces : (handle_idx + 1)
                            * per_handle_faces
                        ]
                        - handle_idx * per_handle_vertices
                    )
                    total_num_vertices[instance_name] += (
                        handle_idx * per_handle_vertices
                    )
            elif "hinge" in module_name.lower():
                existence_of_door = component.existence_of_door
                num_hinge = int(existence_of_door[0] + existence_of_door[1])
                per_hinge_vertices = len(component.vertices) // num_hinge
                per_hinge_faces = len(component.faces) // num_hinge
                assert per_hinge_vertices * num_hinge == len(component.vertices), (
                    f"Number of vertices does not match: {per_hinge_vertices} * {num_hinge} != {len(component.vertices)}"
                )
                for hinge_idx in range(num_hinge):
                    instance_name = f"{component.semantic}_{hinge_idx}"
                    vertices_list.setdefault(instance_name, [])
                    faces_list.setdefault(instance_name, [])
                    total_num_vertices.setdefault(instance_name, 0)
                    ...
                    vertices_list[instance_name].append(
                        component.vertices[
                            hinge_idx * per_hinge_vertices : (hinge_idx + 1)
                            * per_hinge_vertices
                        ]
                    )
                    faces_list[instance_name].append(
                        component.faces[
                            hinge_idx * per_hinge_faces : (hinge_idx + 1)
                            * per_hinge_faces
                        ]
                        - hinge_idx * per_hinge_vertices
                    )
                    total_num_vertices[instance_name] += hinge_idx * per_hinge_vertices
            elif "doorframe" in module_name.lower():
                existence_of_doorframe = component.existence_of_doorframe
                num_doorframe = int(
                    existence_of_doorframe[0] + existence_of_doorframe[1]
                )
                if num_doorframe == 0:
                    num_doorframe = 1

                per_doorframe_vertices = len(component.vertices) // num_doorframe
                per_doorframe_faces = len(component.faces) // num_doorframe
                assert per_doorframe_vertices * num_doorframe == len(
                    component.vertices
                ), (
                    f"Number of vertices does not match: {per_doorframe_vertices} * {num_doorframe} != {len(component.vertices)}"
                )
                for doorframe_idx in range(num_doorframe):
                    instance_name = f"{component.semantic}_{doorframe_idx}"
                    vertices_list.setdefault(instance_name, [])
                    faces_list.setdefault(instance_name, [])
                    total_num_vertices.setdefault(instance_name, 0)
                    ...
                    vertices_list[instance_name].append(
                        component.vertices[
                            doorframe_idx * per_doorframe_vertices : (doorframe_idx + 1)
                            * per_doorframe_vertices
                        ]
                    )
                    faces_list[instance_name].append(
                        component.faces[
                            doorframe_idx * per_doorframe_faces : (doorframe_idx + 1)
                            * per_doorframe_faces
                        ]
                        - doorframe_idx * per_doorframe_vertices
                    )
                    total_num_vertices[instance_name] += (
                        doorframe_idx * per_doorframe_vertices
                    )

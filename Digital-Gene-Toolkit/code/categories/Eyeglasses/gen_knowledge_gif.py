...
def generate_grasp_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, part_names=["leg"]):
    ...
    for concept in concepts:
        if "leg" in concept["template"].lower():
            leg_concept = concept
            break
    assert leg_concept is not None, "No Standard_Door concept found in the list."

    leg_concept["parameters"]["rotation_1"][1] = -0
    leg_rotation = leg_concept["parameters"]["rotation_1"][1]
    min_angle, max_angle = -30, -20  # degree
    total_angle_range = max_angle - leg_rotation
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Eyeglasses", manipulation_knowledge)
    )
    ...
    for i in range(3 * num_frames):
        if i < 2 * num_frames:
            ...
            leg_concept["parameters"]["rotation_1"][1] += angle_per_frame
            ...
            grasp_pose, force_direction, gripper_width_ratio, _ = get_manip_knowledge(
                concepts, "Eyeglasses", manipulation_knowledge
            )
            gripper_mesh = deepcopy(origin_gripper_mesh).transform(grasp_pose)
            vis.add_geometry(gripper_mesh)
            ctr = vis.get_view_control()
...
def generate_affordance_knowledge_gif(
    ...
    leg_rotation_1 = leg_concept["parameters"]["rotation_1"]
    leg_rotation = leg_rotation_1[1]
    min_angle, max_angle = -80, 5  # degree
    total_angle_range = (max_angle - leg_rotation) + (max_angle - min_angle)
    turn_times.append(
        int(np.ceil(num_frames * (max_angle - leg_rotation) / total_angle_range))
    )
    angle_per_frame = total_angle_range / num_frames
    ...
    for i in range(num_frames):
        ...
        for idx in range(len(turn_times)):
            ...
            leg_concept["parameters"]["rotation_1"][1] += direction * angle_per_frame

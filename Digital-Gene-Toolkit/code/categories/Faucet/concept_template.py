...
from code.utils import apply_transformation, get_rodrigues_matrix
...
class Cuboidal_Base(ConceptTemplate):
    def __init__(
        ...
        self.back_mesh = Cuboid(
            size_0[1], size_0[0], size_0[2], position=back_mesh_position
        )
        ...
        if number_of_box[0] == 2:
            ...
            self.back_mesh = Cuboid(
                size_1[1], size_1[0], size_1[2], position=back_mesh_position
            )
            vertices_list.append(self.back_mesh.vertices)
            faces_list.append(self.back_mesh.faces + total_num_vertices)
            total_num_vertices += len(self.back_mesh.vertices)
        ...
        self.vertices = apply_transformation(
            self.vertices, position, rotation, rotation_order="YXZ", offset_first=True
        )
...
class Cylindrical_Base(ConceptTemplate):
    def __init__(
        ...
        self.back_mesh = Cylinder(
            size_0[1], size_0[0], size_0[0], position=back_mesh_position
        )
        vertices_list.append(self.back_mesh.vertices)
        faces_list.append(self.back_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.back_mesh.vertices)
        ...
        if number_of_cylinder[0] == 2:
            ...
            self.back_mesh = Cylinder(
                size_1[1], size_1[0], size_1[0], position=back_mesh_position
            )
            vertices_list.append(self.back_mesh.vertices)
            faces_list.append(self.back_mesh.faces + total_num_vertices)
            total_num_vertices += len(self.back_mesh.vertices)
...
class UShapedXZ_Base(ConceptTemplate):
    def __init__(self, R, size_tube, size_base, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        tmp_mesh_position = [-size_tube[0] / 2, 0, size_base[1] + size_tube[1] / 2]
        tmp_mesh_rotation = [np.pi / 2, 0, 0]
        self.tmp_mesh = Cylinder(
            size_tube[1],
            R[0],
            R[0],
...
class UShapedYZ_Base(ConceptTemplate):
    def __init__(self, R, size_tube, size_base, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.tmp_mesh = Cylinder(size_tube[1], R[0], R[0], position=tmp_mesh_position)
        vertices_list.append(self.tmp_mesh.vertices)
        faces_list.append(self.tmp_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.tmp_mesh.vertices)
        ...
        tmp_mesh_rotation = [0, 0, np.pi / 2]
        self.tmp_mesh = Cylinder(size_tube[0], R[0], R[0], rotation=tmp_mesh_rotation)
        vertices_list.append(self.tmp_mesh.vertices)
        faces_list.append(self.tmp_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.tmp_mesh.vertices)
        ...
        self.tmp_mesh = Cylinder(
            size_base[1], size_base[0], size_base[0], position=tmp_mesh_position
        )
        vertices_list.append(self.tmp_mesh.vertices)
        faces_list.append(self.tmp_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.tmp_mesh.vertices)

        tmp_mesh_position = [-size_tube[0] / 2, -(size_base[1] / 2 + size_tube[1]), 0]
        self.tmp_mesh = Cylinder(
            size_base[1], size_base[0], size_base[0], position=tmp_mesh_position
        )
        vertices_list.append(self.tmp_mesh.vertices)
...
class Trifold_Spout(ConceptTemplate):
    def __init__(
        ...
        for start_end in start_ends:
            ...
            tmp_mesh = Cylinder(length, R[0], R[0], position=tmp_mesh_position)
...
class Curved_Spout(ConceptTemplate):
    def __init__(
        ...
        torus_mesh_rotation = [-np.pi / 2, np.arctan(center[0] / center[2]), np.pi / 2]
        self.torus_mesh = Torus(
            r,
            R[0],
        ...
        self.torus_mesh.vertices = np.matmul(
            ...
            get_rodrigues_matrix(
                ...
                -np.arctan(
                    center[1] / (center[2] / np.cos(np.arctan(center[0] / center[2])))
                ),
            ).T,
        )

        self.torus_mesh.vertices += (
            np.array(bottom0) + np.array(bottom1) + np.array(center)
        )
        ...
        self.end_mesh = Cylinder(end_length, R[0], R[0], position=end_mesh_position)
        self.end_mesh.vertices = apply_transformation(
            self.end_mesh.vertices, [0, 0, 0], [0, np.arctan(center[0] / center[2]), 0]
        )
...
class Cuboidal_Spout(ConceptTemplate):
    def __init__(
        ...
        self.head_mesh = Cylinder(
            head_size[1], head_size[0], head_size[0], position=head_mesh_position
        )
...
class SimplifiedZ_Switch(ConceptTemplate):
    def __init__(
        ...
        for offset in self.offsets:
            tmp_mesh_rotation = [np.pi / 2, 0, 0]
            tmp_mesh = Cylinder(
                size[1], size[0], size[0], position=offset, rotation=tmp_mesh_rotation
            )
            vertices_list.append(tmp_mesh.vertices)
            faces_list.append(tmp_mesh.faces + total_num_vertices)
            total_num_vertices += len(tmp_mesh.vertices)
    ...
    def __init__(
        ...
        for offset in self.offsets:
            for i in range(number_of_cylinder[0]):
                size = self.sizes[i]
                tmp_mesh = Cylinder(size[1], size[0], size[0], position=offset[i])
                vertices_list.append(tmp_mesh.vertices)
                faces_list.append(tmp_mesh.faces + total_num_vertices)
                total_num_vertices += len(tmp_mesh.vertices)
    ...
    def __init__(
        ...
        for offset in self.offsets:
            for i in range(number_of_cube[0]):
                size = self.sizes[i]
                tmp_mesh = Cuboid(size[1], size[0], size[2], position=offset[i])
                vertices_list.append(tmp_mesh.vertices)
                faces_list.append(tmp_mesh.faces + total_num_vertices)
                total_num_vertices += len(tmp_mesh.vertices)
    ...
    def __init__(
        ...
        for offset in self.offsets:
            for i in range(number_of_cube[0]):
                size = self.sizes[i]
                tmp_mesh = Cuboid(
                    size[1], size[0], size[2], position=offset[i], rotation=[0, 0, 0]
                )
                vertices_list.append(tmp_mesh.vertices)
                faces_list.append(tmp_mesh.faces + total_num_vertices)
                total_num_vertices += len(tmp_mesh.vertices)
...
class RegularX_Switch(ConceptTemplate):
    def __init__(
        ...
        self.sub_mesh = Cylinder(
            sub_size[1], sub_size[0], sub_size[0], position=sub_mesh_position
        )
        self.sub_mesh.vertices = apply_transformation(
            self.sub_mesh.vertices,
            [
        ...
        self.main_mesh = Cylinder(
            size[1], size[0], size[0], rotation=main_mesh_rotation
        )
        self.main_mesh.vertices = apply_transformation(
            self.main_mesh.vertices,
            [size[1] / 2, 0, 0],
...
class TShaped_Switch(ConceptTemplate):
    def __init__(
        ...
        size = self.sizes[0]
        self.tmp_mesh = Cylinder(size[1], size[0], size[0], position=self.offsets[0])
        vertices_list.append(self.tmp_mesh.vertices)
        faces_list.append(self.tmp_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.tmp_mesh.vertices)

        size = self.sizes[1]
        self.tmp_mesh = Cylinder(size[1], size[0], size[0], position=self.offsets[1])
        vertices_list.append(self.tmp_mesh.vertices)
        faces_list.append(self.tmp_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.tmp_mesh.vertices)

        size = self.sizes[2]
        self.tmp_mesh = Cuboid(size[1], size[0], size[2], position=self.offsets[2])
        self.tmp_mesh.vertices = apply_transformation(
            self.tmp_mesh.vertices, [0, 0, 0], [0, switch_rotation[0], 0]
        )
        vertices_list.append(self.tmp_mesh.vertices)
        faces_list.append(self.tmp_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.tmp_mesh.vertices)
...
class Lever_Switch(ConceptTemplate):
    def __init__(
        ...
        tmp_mesh = Cylinder(size[1], size[0], size[0], position=tmp_mesh_position)
        vertices_list.append(tmp_mesh.vertices)
        faces_list.append(tmp_mesh.faces + total_num_vertices)
        total_num_vertices += len(tmp_mesh.vertices)
...
class RotaryX_Switch(ConceptTemplate):
    def __init__(
        ...
        for existence in existences:
            ...
            tmp_mesh_position = [
                offset_x[j]
                + existence * (interval / 2 + main_size_1[1] + main_size_2[1] / 2),
                0,
                0,
            ]
            tmp_mesh_rotation = [0, 0, np.pi / 2]

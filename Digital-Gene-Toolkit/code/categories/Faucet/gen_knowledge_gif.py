...
def generate_grasp_knowledge_gif(
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Faucet", manipulation_knowledge)
    )
    ...
    template_name = switch_concept["template"]

    if "Rotary" in template_name:
        rotation0 = switch_concept["parameters"]["rotation0"][0]
        rotation1 = switch_concept["parameters"]["rotation1"][0]
        switch_rotations = [rotation0, rotation1]
    elif "Regular" in template_name:
        sub_rotation = switch_concept["parameters"]["sub_rotation"][0]
        switch_rotations = [sub_rotation]
    elif template_name == "Lever_Switch":
        actionable_parts_info = analyze_actionable_parts_from_conceptualization(
            concepts
        )
    ...
    elif template_name == "HandleY_Switch":
        raise GenGifError("HandleY_Switch is not movable")
    else:
        raise GenGifError(f"Unsupported switch template: {template_name}")

    rot_steps = []
    turn_times = []
    if template_name == "Lever_Switch":
        ...
        min_angle, max_angle = -30, 30  # degree
        for switch_rotation in switch_rotations:
            ...
            turn_times.append(
                int(
                    np.ceil(
                        num_frames * (max_angle - switch_rotation) / total_angle_range
                    )
                )
            )
            angle_per_frame = total_angle_range / num_frames
        ...
        for i in range(3 * num_frames):
            if i < 2 * num_frames:
                ...
                for idx in range(len(turn_times)):
                    ...
                    if "Rotary" in template_name:
                        switch_concept["parameters"]["rotation0"][0] += (
                            direction * angle_per_frame
                        )
                        switch_concept["parameters"]["rotation1"][0] += (
                            direction * angle_per_frame
                        )
                    elif "Regular" in template_name:
                        switch_concept["parameters"]["sub_rotation"][0] += (
                            direction * angle_per_frame
                        )
                    else:
                        raise ValueError(
                            f"Unsupported switch template: {template_name}"
                        )
...
def generate_affordance_knowledge_gif(
    ...
    if "Rotary" in template_name:
        ...
        print("HandleY_Switch is not movable")
        return
    else:
        raise GenGifError(f"Unsupported switch template: {template_name}")
        ...
        for i in range(num_frames):
            ...
            for idx in range(len(turn_times)):
                ...
                if "Rotary" in template_name:
                    ...
                    raise ValueError(f"Unsupported switch template: {template_name}")

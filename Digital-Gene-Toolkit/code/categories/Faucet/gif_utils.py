...
def assemble_concepts_instance(concepts):
    ...
    for c in concepts:
        ...
        try:
            ...
            if module_name in ["RegularY_Switch", "Lever_Switch"]:
                num_instance = 1
            elif "rotaryx" in module_name.lower():  # rotationaryX HandleX
                existence_of_switch = component.existence_of_switch
                num_instance = int(np.sum(existence_of_switch))
            elif "switch" in module_name.lower():  # rotationaryY HandleY
                if hasattr(component, "number_of_switch"):
                    number_of_switch = component.number_of_switch
                    num_instance = number_of_switch[0]
                else:
                    num_instance = 1
            else:
                num_instance = 1
...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    # find switch_concept and magazine_concept
    switch_concept = find_concept_by_template_name("switch")
    switch_position = switch_concept["parameters"]["position"]

    template = switch_concept["template"]
    assert "Lever_Switch" == template, (
        f"Expected template 'Lever_Switch', but got '{template}'."
    )
    size = switch_concept["parameters"]["size"]
    axis_pos = np.array([0, size[1] / 2, 0], dtype=np.float32)
    axis_pos += np.array(switch_position)
    rotation_axis = np.array([0, 1, 0], dtype=np.float32)

    move_state_and_limit = np.array([-30, 30, 0])

    actionable_parts_info["template_names"] = [
        switch_concept["template"],
    ]
    ...
    actionable_parts_info["move_state_and_limits"] = [move_state_and_limit]

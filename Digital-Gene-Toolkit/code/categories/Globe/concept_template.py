...
class Semi_Ring_Bracket(ConceptTemplate):
    def __init__(
        ...
        if pivot_continuity[0] == 1:
            ...
            self.top_pivot_mesh = Cylinder(
                pivot_size[1], pivot_size[0], position=top_pivot_mesh_position
            )
            ...
            self.bottom_pivot_mesh = Cylinder(
                pivot_size[1], pivot_size[0], position=bottom_pivot_mesh_position
            )
        ...
        if has_top_endpoint[0] == 1:
            ...
            self.top_endpoint_mesh = Sphere(
                endpoint_radius[0], position=top_endpoint_mesh_position
            )
        ...
        if has_bottom_endpoint[0] == 1:
            ...
            self.bottom_endpoint_mesh = Sphere(
                endpoint_radius[0], position=bottom_endpoint_mesh_position
            )
        ...
        self.bracket_mesh = Ring(
            bracket_size[2],
            bracket_size[0],
            bracket_size[1],
            bracket_exist_angle[0],
...
class Tilted_Bracket(ConceptTemplate):
    def __init__(
        ...
        self.pivot_mesh = Cylinder(
            pivot_size[1], pivot_size[0], rotation=pivot_mesh_rotation
        )
        vertices_list.append(self.pivot_mesh.vertices)
        faces_list.append(self.pivot_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.pivot_mesh.vertices)

        bracket_mesh_rotation = [np.pi / 2, np.pi / 2, 0]
        self.bracket_mesh = Ring(
            bracket_size[2],
            bracket_size[0],
            bracket_size[1],
        ...
        self.circle_mesh = Ring(
            circle_thickness[1],
            bracket_size[1],
            bracket_size[1] - circle_thickness[0],
...
class Enclosed_Bracket(ConceptTemplate):
    def __init__(
        ...
        if half_circle_number[0] == 2:
            bracket_mesh_rotation = [np.pi / 2, 0, 0]
            self.bracket_mesh = Ring(
                bracket_size[2],
                bracket_size[0],
                bracket_size[1],
...
class Cylindrical_Base(ConceptTemplate):
    def __init__(self, bottom_size, top_size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        top_mesh_position = [0, -top_size[2] / 2, 0]
        self.top_mesh = Cylinder(
            top_size[2], top_size[0], top_size[1], position=top_mesh_position
        )
        ...
        bottom_mesh_position = [0, -top_size[2] - bottom_size[2] / 2, 0]
...
class Cuboidal_Base(ConceptTemplate):
    def __init__(self, bottom_size, top_size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        top_mesh_position = [0, -top_size[1] / 2, 0]
        ...
        bottom_mesh_position = [0, -top_size[1] - bottom_size[1] / 2, 0]
...
class Star_Shaped_Base(ConceptTemplate):
    def __init__(
        ...
        self.num_legs = [round(item) for item in num_legs]
        ...
        top_mesh_position = [0, -top_size[1] / 2, 0]
        self.top_mesh = Cylinder(top_size[1], top_size[0], position=top_mesh_position)
        vertices_list.append(self.top_mesh.vertices)
        faces_list.append(self.top_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.top_mesh.vertices)

        for i in range(self.num_legs[0]):
            rot = np.pi * 2 / num_legs[0] * i
            claw_mesh_rotation = [tilt_angle[0], -rot, 0]
            x_ = sub_size[2] / 2 * np.cos(tilt_angle[0]) * np.sin(rot)
            z_ = sub_size[2] / 2 * np.cos(tilt_angle[0]) * np.cos(rot)
            y__ = (
                -top_size[1] + sub_size[1] / 2 - sub_size[2] * np.sin(tilt_angle[0]) / 2
            )
            claw_mesh_position = [-x_, y__, z_]
            self.claw_mesh = Cuboid(
                sub_size[1],
                sub_size[0],
                sub_size[2],
...
class Special_Base(ConceptTemplate):
    def __init__(
        self, radius, top_size, top_rotation, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        ...
        self.bottom_mesh = Torus(radius[0], radius[1], position=bottom_mesh_position)
        vertices_list.append(self.bottom_mesh.vertices)
        faces_list.append(self.bottom_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.bottom_mesh.vertices)
...
class Table_Like_Base(ConceptTemplate):
    def __init__(
        ...
        for i in range(self.num_legs[0]):
            ...
            self.leg_mesh = Cylinder(
                leg_size[1], leg_size[0], position=leg_mesh_position
            )
            ...
            if has_bottom_part[0] == 1:
                ...
                bottom_mesh_rotation = [0, -rotation_tmp, 0]
                self.bottom_mesh = Cuboid(
                    bottom_size[1],
                    bottom_size[0],
                    bottom_size[2],

...
def generate_grasp_knowledge_gif(
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Globe", manipulation_knowledge)
    )
...
def main():
    category = "Globe"
    conceptualization = pickle.load(open(f"{category}.pkl", "rb"))

    for idx, c in enumerate(conceptualization[:20]):
        template_names = [cc["template"] for cc in c["conceptualization"]]
        print(f"Processing {idx}th conceptualization, template names: {template_names}")
        concepts = c["conceptualization"]
        # json.dump(concepts, open(f"{category}_concepts.json", "w"), indent=4)

        # output_path = f"output_gifs/{category.lower()}_knowledge_{idx}.gif"
        # generate_knowledge_gif(
        #     concepts,
        ...
        # output_path = f"output_gifs/{category.lower()}_instance_knowledge_{idx}.gif"
        ...
        output_path = f"output_gifs/{category.lower()}_grasp_knowledge_{idx}.gif"

...
from code.utils import apply_transformation
...
class Semi_Spherical_Body(ConceptTemplate):
    def __init__(
        ...
        self.top_mesh = Sphere(
            horizontal_axis[0] * x_z_ratio[0],
            np.pi / 2 - exist_angle[0],
            np.pi / 2,
        ...
        self.middle_mesh = Sphere(
            horizontal_axis[0] * x_z_ratio[0],
            np.pi / 2 - exist_angle[1],
            np.pi / 2,
        ...
        bottom_offset = (
            horizontal_axis[0]
            * np.sin(exist_angle[1])
            * vertical_axis[1]
            / horizontal_axis[0]
        )
        bottom_mesh_position = [0, -bottom_offset - bottom_size[1] / 2, 0]
        self.bottom_mesh = Cylinder(
            bottom_size[1],
            bottom_radius * x_z_ratio[0],
            bottom_size[0] * x_z_ratio[0],
...
class Spherical_Cylindrical_Body(ConceptTemplate):
    def __init__(
        ...
        bottom_mesh_position = [0, -bottom_size[1] / 2, 0]
        self.bottom_mesh = Cylinder(
            bottom_size[1],
            horizontal_axis[0] * x_z_ratio[0],
            bottom_size[0] * x_z_ratio[0],
...
class Multilevel_Body(ConceptTemplate):
    def __init__(
        ...
        level_1_top_radius,
        level_1_height,
        level_2_top_radius=0,
        level_2_height=0,
        level_3_top_radius=0,
        level_3_height=0,
        level_4_top_radius=0,
        level_4_height=0,
        level_5_top_radius=0,
        level_5_height=0,
        x_z_ratio=1,
        position=[0, 0, 0],
        rotation=[0, 0, 0],
        ...
        mesh_1_top_radius = (
            level_1_top_radius[0] * (1 - level_1_height[1] / level_1_height[0])
            + level_1_bottom_radius[0] * level_1_height[1] / level_1_height[0]
        )
        mesh_1_height = level_1_height[0] - level_1_height[1]
        bottom_mesh_position = [0, -level_1_height[1] / 2, 0]
        self.bottom_mesh = Cylinder(
            mesh_1_height,
            mesh_1_top_radius * x_z_ratio[0],
            level_1_bottom_radius[0] * x_z_ratio[0],
        ...
        self.top_mesh = Ring(
            level_1_height[1],
            level_1_top_radius[0],
            level_1_top_radius[1],
        ...
        for i in range(num_levels[0] - 1):
            ...
            delta_height += locals()["level_" + str(i + 2) + "_height"][0] / 2
            self.top_mesh = Ring(
                locals()["level_" + str(i + 2) + "_height"][0],
                locals()["level_" + str(i + 2) + "_top_radius"][0],
                locals()["level_" + str(i + 2) + "_top_radius"][1],
...
class Standard_Cover(ConceptTemplate):
    def __init__(
        ...
        mesh_1_bottom_radius = (
            outer_size[1] * (1 - inner_size[2] / outer_size[2])
        ...
        for i in range(num_knobs[0]):
            ...
            self.knob_mesh = Cylinder(
                locals()["knob_%d_size" % (i + 1)][2],
                locals()["knob_%d_size" % (i + 1)][0],
                locals()["knob_%d_size" % (i + 1)][1],
...
class Trifold_Handle(ConceptTemplate):
    def __init__(
        ...
        bottom_mesh_position = [
            0,
            -horizontal_separation[0] / 2
            - horizontal_length[1] * np.sin(horizontal_rotation[1]) / 2,
...
class Curved_Handle(ConceptTemplate):
    def __init__(self, radius, exist_angle, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.mesh = Torus(radius[0], radius[1], exist_angle[0], rotation=mesh_rotation)
...
class Ring_Handle(ConceptTemplate):
    def __init__(self, size, exist_angle, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        mesh_rotation = [np.pi, 0, -np.pi / 2]
        self.mesh = Ring(
            size[2], size[0], size[1], exist_angle[0], rotation=mesh_rotation
        )
        vertices_list.append(self.mesh.vertices)
        faces_list.append(self.mesh.faces + total_num_vertices)
        total_num_vertices += len(self.mesh.vertices)
...
class Cylindrical_Handle(ConceptTemplate):
    def __init__(self, size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.mesh = Cylinder(
            size[2], size[0], size[1], position=mesh_position, rotation=mesh_rotation
        )
        vertices_list.append(self.mesh.vertices)
        faces_list.append(self.mesh.faces + total_num_vertices)
        total_num_vertices += len(self.mesh.vertices)
...
class Round_U_Handle(ConceptTemplate):
    def __init__(
        ...
        left_mesh_position = [0, vertical_length[0] / 2, vertical_separation[0] / 2]
        self.left_mesh = Cylinder(
            vertical_length[0], mounting_radius[0], position=left_mesh_position
        )
        ...
        right_mesh_position = [0, vertical_length[0] / 2, -vertical_separation[0] / 2]
        self.right_mesh = Cylinder(
            vertical_length[0], mounting_radius[0], position=right_mesh_position
        )
...
class Flat_U_Handle(ConceptTemplate):
    def __init__(
        self, vertical_size, vertical_separation, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        super().__init__(position, rotation)
        ...
        left_mesh_position = [0, vertical_size[1] / 2, vertical_separation[0] / 2]
        self.left_mesh = Cuboid(
            vertical_size[1],
            vertical_size[0],
            vertical_size[2],
        ...
        right_mesh_position = [0, vertical_size[1] / 2, -vertical_separation[0] / 2]
        self.right_mesh = Cuboid(
            vertical_size[1],
            vertical_size[0],
            vertical_size[2],
        ...
        curve_mesh_position = [0, vertical_size[1], 0]
        curve_mesh_rotation = [-np.pi / 2, np.pi / 2, 0]
        self.curve_mesh = Ring(
            vertical_size[0],
            outer_radius,
            mounting_radius,
            np.pi,
...
class Straight_Spout(ConceptTemplate):
    def __init__(
        ...
        total_delta_y = (
            -(spout_1_length[0] + spout_1_length[1]) / 4 * np.sin(spout_1_rotation[0])
        )
        total_delta_z = (
            -(spout_1_length[0] + spout_1_length[1]) / 4 * np.cos(spout_1_rotation[0])
        )

        for i in range(num_of_sub_spouts[0]):
            total_delta_y += (
                (
                    locals()["spout_%d_length" % (i + 1)][0]
                    + locals()["spout_%d_length" % (i + 1)][1]
                )
                / 4
                * np.sin(locals()["spout_%d_rotation" % (i + 1)][0])
            )
            total_delta_z += (
                (
                    locals()["spout_%d_length" % (i + 1)][0]
                ...
                / 4
                * np.cos(locals()["spout_%d_rotation" % (i + 1)][0])
            )
            ...
            self.mesh = Ring(
                height=locals()["spout_%d_length" % (i + 1)][0],
                outer_top_radius=locals()["spout_%d_radius" % (i + 1)][0],
                inner_top_radius=locals()["spout_%d_radius" % (i + 1)][0]
                - locals()["spout_%d_thinkness" % (i + 1)][0] * 2,
                outer_bottom_radius=locals()["spout_%d_radius" % (i + 1)][1],
                inner_bottom_radius=locals()["spout_%d_radius" % (i + 1)][1]
                - locals()["spout_%d_thinkness" % (i + 1)][0] * 2,
...
class Curved_Spout(ConceptTemplate):
    def __init__(
        ...
        top_mesh_position = [0, central_radius[0], 0]
        ...
        bottom_mesh_position = [
            0,
            central_radius[0] * (1 - np.cos(exist_angle[0]))
            - central_radius[1] * np.cos(exist_angle[0]),
            central_radius[0] * np.sin(exist_angle[0])
            + central_radius[1] * np.sin(exist_angle[0]),
        ]
        bottom_mesh_rotation = [-exist_angle[0], 0, np.pi / 2]
        self.bottom_mesh = Torus(
            central_radius[1],
            torus_radius[1],
            exist_angle[1],
            torus_radius[2],

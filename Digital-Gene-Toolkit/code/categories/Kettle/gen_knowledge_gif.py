...
def generate_grasp_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, part_names=["handle", "cover"]):
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Kettle", manipulation_knowledge)
    )
    ...
    for concept in concepts:
        if "cover" in concept["template"].lower():
            cover_concept = concept
            break

    tophandle_concept = None
    for concept in concepts:
        if template_names[0].lower() == concept["template"].lower():
            tophandle_concept = concept
            break
    sidehandle_concept = None
    for concept in concepts:
        if "handle" in concept["template"].lower():
            if concept["template"].lower() != template_names[0].lower():
                sidehandle_concept = concept
                break
    ...
    gripper_frames_num = num_frames
    gripper_params = []
    cur_gripper_param = -1
    per_frame_delta = 2 / gripper_frames_num
    for i in range(gripper_frames_num):
        gripper_params.append([cur_gripper_param] * 3)
        cur_gripper_param += per_frame_delta

    angle_per_frame = 180 / num_frames
    R = Rot.from_euler("y", angles=angle_per_frame, degrees=True).as_matrix()[:3, :3] 

    images = []
    move_steps = {}
    turn_times = {}
    for template_name, move_type, move_center, move_axis, move_state_and_limit in zip(
        template_names, move_types, move_centers, move_axises, move_state_and_limits
    ):
        if move_type == "prismatic":
            min_height, max_height, cur_height = move_state_and_limit
            assert min_height <= cur_height, "move_state_and_limit1"
            assert cur_height <= max_height, "move_state_and_limit2"
            total_height_range = (max_height - cur_height) + (max_height - min_height)
            turn_times[template_name] = int(
                np.ceil(num_frames * (max_height - cur_height) / total_height_range)
            )
            height_per_frame = total_height_range / num_frames
            # up
            translate = np.array([0, 0, 0], dtype=np.float64)
            axis_idx = np.argmax(np.abs(move_axis))
            translate[axis_idx] = height_per_frame
            move_steps[template_name] = translate

        elif move_type == "revolute":
            min_angle, max_angle, cur_angle = move_state_and_limit
            assert min_angle <= cur_angle, "move_state_and_limit3"
            assert cur_angle <= max_angle, "move_state_and_limit4"
            total_angle_range = (max_angle - cur_angle) + (max_angle - min_angle)
            turn_times[template_name] = int(
                np.ceil(num_frames * (max_angle - cur_angle) / total_angle_range)
            )
            angle_per_frame = total_angle_range / num_frames
            move_axis = move_axis / np.linalg.norm(move_axis)  
            rot = Rot.from_rotvec(np.radians(angle_per_frame) * np.array(move_axis))
            move_steps[template_name] = rot.as_matrix()[:3, :3]
        else:
            raise ValueError(f"Unknown move type: {move_type}")

    if cover_concept != None:
        for i in range(num_frames):
    ...
    if tophandle_concept != None:
        for i in range(num_frames, 2 * num_frames):
            vis.clear_geometries()
            for mesh in part_meshes.values():
                vis.add_geometry(mesh)
            manip_params_size = 2
            if tophandle_concept["template"].lower() == "trifold_handle":
                manip_params_size = 3
            ...
            grasp_pose, force_direction, gripper_width_ratio, _ = get_manip_knowledge(
                ...
                "Kettle",
                manipulation_knowledge,
                gripper_params[i - num_frames][:manip_params_size],
            )
            gripper_mesh = deepcopy(origin_gripper_mesh).transform(grasp_pose)
    ...
    if sidehandle_concept != None:
        for i in range(2 * num_frames, 3 * num_frames):
            vis.clear_geometries()
            for mesh in part_meshes.values():
                vis.add_geometry(mesh)
            manip_params_size = 2
            if sidehandle_concept["template"].lower() == "trifold_handle":
                manip_params_size = 3
            ...
            grasp_pose, force_direction, gripper_width_ratio, _ = get_manip_knowledge(
                ...
                "Kettle",
                manipulation_knowledge,
                gripper_params[i - 2 * num_frames][:manip_params_size],
            )
            gripper_mesh = deepcopy(origin_gripper_mesh).transform(grasp_pose)
...
def generate_affordance_knowledge_gif(
    ...
    for i in range(num_frames * 2):
        for semantic, mesh in part_meshes.items():
            ...
            if "cover" in template_name.lower():
                if i > num_frames:
                    direction = 1 if i % num_frames < turn_times[template_name] else -1
                    translate = move_steps[template_name] * direction
                    mesh.translate(translate, relative=True)
            elif "handle" in template_name.lower():
                if i < num_frames:
                    assert template_idx == 1, "Handle template index should be 1"
                    R = move_steps[template_name]
                    if i % num_frames > turn_times[template_name]:
                        R = R.T
                    mesh.rotate(R, center=move_centers[template_idx])
            else:
                raise ValueError(f"Unknown semantic: {template_name}")
            vis.update_geometry(mesh)

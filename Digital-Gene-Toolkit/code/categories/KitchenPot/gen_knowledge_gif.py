...
def generate_grasp_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, part_names=["tophandle"]):
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "KitchenPot", manipulation_knowledge)
    )
    ...
    if "mutilevel" in template_names[1].lower():
        cur_gripper_param = -1
        per_frame_delta = 2 / gripper_frames_num
        for i in range(len(gripper_params)):
            gripper_params[i][-1] = cur_gripper_param
            cur_gripper_param += per_frame_delta
    ...
    for move_axis, move_state_and_limit in zip(move_axises, move_state_and_limits):
        ...
        turn_times.append(
            int(np.ceil(num_frames * (max_height - cur_height) / total_height_range))
        )
        height_per_frame = total_height_range / num_frames
        translate = np.array([0, height_per_frame, 0], dtype=np.float64)
        translate_steps.append(translate)
    ...
    for i in range(3 * num_frames):
        if i < 2 * num_frames:
            ...
            for semantic, mesh in part_meshes.items():
                ...
                translate = translate_steps[template_idx] * direction
                mesh.translate(translate, relative=True)
                if semantic.lower() == "tophandle":
                    gripper_mesh.translate(translate, relative=True)
                vis.update_geometry(mesh)

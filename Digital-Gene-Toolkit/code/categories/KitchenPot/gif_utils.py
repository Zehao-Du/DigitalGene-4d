...
def assemble_concepts(concepts):
    ...
    for c in concepts:
        ...
        try:
            ...
            if "tophandle" in module_name.lower():
                component.semantic = "tophandle"
            elif "sidehandle" in module_name.lower():
                component.semantic = "sidehandle"
...
def assemble_concepts_instance(concepts):
    ...
    for c in concepts:
        ...
        try:
            module = eval(module_name)
            component = module(**c["parameters"])
            if "sidehandle" in module_name.lower():
                component.semantic = "sidehandle"
            elif "tophandle" in module_name.lower():
                component.semantic = "tophandle"

            if (
                "sidehandle" in module_name.lower()
                and module_name != "Cuboidal_Sidehandle"
            ):
                num_handle = 2
...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    tophandle_concept = find_concept_by_template_name("tophandle")
    body_concept = find_concept_by_template_name("body")
    cover_pos = np.array(cover_concept["parameters"]["position"])
    tophandle_pos = np.array(tophandle_concept["parameters"]["position"])

    body_params = body_concept["parameters"]
    body_top_height = body_params["position"][1]
    body_top_height += body_params["outer_size"][2] / 2

    if cover_concept["template"] == "Cylindrical_Cover":
        lid_radius = cover_concept["parameters"]["size"][0]
        body_top_height += cover_concept["parameters"]["size"][1]
    elif cover_concept["template"] == "Semi_Spherical_Cover":
        lid_radius = cover_concept["parameters"]["radius"][0]
    elif cover_concept["template"] == "Carved_Cylindrical_Cover":
        lid_radius = cover_concept["parameters"]["outer_size"][1]
        body_top_height += cover_concept["parameters"]["outer_size"][2]
    else:
        raise GenGifError("Unknown cover type: {}".format(cover_concept["template"]))

    move_state_and_limits = []
    cover_state_and_limits = np.array(
        [body_top_height, cover_pos[1] + lid_radius, cover_pos[1]]
    )
    tophandle_state_and_limits = cover_state_and_limits.copy()
    tophandle_state_and_limits += tophandle_pos[1] - cover_pos[1]
    move_state_and_limits.append(cover_state_and_limits)
    move_state_and_limits.append(tophandle_state_and_limits)
    actionable_parts_info["template_names"] = [
        cover_concept["template"],
        tophandle_concept["template"],
    ]
    actionable_parts_info["move_types"] = ["prismatic", "prismatic"]
    actionable_parts_info["move_centers"] = [cover_pos[1], cover_pos[1]]
    actionable_parts_info["move_axises"] = [[0,1,0], [0,1,0]]

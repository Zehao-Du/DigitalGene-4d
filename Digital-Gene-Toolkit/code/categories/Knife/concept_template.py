...
class Curved_Handle(ConceptTemplate):
    def __init__(
        self, radius, thickness, exist_angle, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
...
class Multideck_Handle(ConceptTemplate):
    def __init__(
        ...
        bottom_mesh_position = [0, -beside_size[1] / 2, 0]
        self.bottom_mesh = Cuboid(
            bottom_size[1],
            bottom_size[0],
            bottom_size[2],
        ...
        self.left_mesh = Cuboid(
            beside_size[1], beside_size[0], beside_size[2], position=left_mesh_position
        )
        ...
        self.right_mesh = Cuboid(
            beside_size[1], beside_size[0], beside_size[2], position=right_mesh_position
        )
...
class Enveloping_Handle(ConceptTemplate):
    def __init__(
        self, size, thickness, gap_width, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        super().__init__(position, rotation)
        ...
        bottom_mesh_position = [0, -(size[1] - thickness[0]) / 2, 0]
        self.bottom_mesh = Cuboid(
            thickness[0], size[0], size[2], position=bottom_mesh_position
        )
        vertices_list.append(self.bottom_mesh.vertices)
        faces_list.append(self.bottom_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.bottom_mesh.vertices)

        behind_mesh_position = [-(size[0] - thickness[1]) / 2, thickness[0] / 2, 0]
        self.behind_mesh = Cuboid(
            size[1] - thickness[0], thickness[1], size[2], position=behind_mesh_position
        )
        ...
        self.left_front_mesh = Cuboid(
            size[1] - thickness[0],
            thickness[2],
            (size[2] - gap_width[0]) / 2 - thickness[2],
...
class Cusp_Blade(ConceptTemplate):
    def __init__(
        ...
        mesh_position = [0, root_size[1] / 2, 0]
        self.mesh = Cuboid(
            root_size[1],
            root_size[0],
            root_size[3],
        ...
        tip_mesh_position = [0, root_size[1] + tip_length[0] / 2, root_z_offset[0]]
...
class Curved_Blade(ConceptTemplate):
    def __init__(
        ...
        tip_mesh_position = [0, root_size[1], root_z_offset[0] - root_size[3] / 2]
        tip_mesh_rotation = [0, 0, np.pi / 2]
        self.tip_mesh = Cylinder(
            root_size[0],
            tip_length[0],
            tip_length[0],
            root_size[3],

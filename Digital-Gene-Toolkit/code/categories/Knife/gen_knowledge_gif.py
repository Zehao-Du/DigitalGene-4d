...
def generate_grasp_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, ["handle","button"]):
        raise GenGifError(
            "No actionable parts found in the concepts. Please check the input data."
        )
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Knife", manipulation_knowledge)
    )
    ...
    cur_gripper_param = -1
    for i in range(gripper_frames_num):
        gripper_side_params.append(cur_gripper_param)
        cur_gripper_param += per_frame_delta
    ...
    for i in range(2 * num_frames):
        ...
        grasp_pose, force_direction, gripper_width_ratio, _ = get_manip_knowledge(
            ...
            [gripper_params[i]] * (manip_params_size - 1) + [gripper_side_params[i]],
        )
...
def generate_affordance_knowledge_gif(
    ...
    template_names = [c["template"] for c in concepts]
    if not (
        "Multideck_Handle" in template_names or "Enveloping_Handle" in template_names
    ):
        raise GenGifError(
            "No actionable parts found in the concepts. Please check the input data."
        )
    ...
    # look at the handle from the front
    if "Enveloping_Handle" in [c["template"] for c in concepts]:
        current_camera_params = vis.get_view_control().convert_to_pinhole_camera_parameters()
        extrinsic = current_camera_params.extrinsic
        # Extract rotation matrix R and translation vector t
        R = extrinsic[:3, :3]
        t = extrinsic[:3, 3]
        # Compute the camera position in world coordinates: camera_pos = -R^T @ t
        camera_pos = -R.T @ t
        # Compute the forward direction vector of the camera (negative Z-axis in Open3D)
        forward_vector = -R.T @ np.array([0, 0, 1])
        # Compute a look-at point located in front of the camera
        lookat_point = camera_pos + forward_vector * 10.0  # Multiply by a distance factor
        # Flip the camera position across the X-Z plane (equivalent to rotating 180 degrees around the Y-axis)
        new_camera_pos = np.array([-camera_pos[0], camera_pos[1], -camera_pos[2]])
        # Create a new extrinsic matrix with the updated camera position, keeping the up direction unchanged
    ...
    is_rotate = "revolute" in move_types
    ...
    for move_axis, move_state_and_limit in zip(move_axises, move_state_and_limits):
        ...
        if is_rotate:
            rot = Rot.from_rotvec(dist_per_frame * move_axis, degrees=True)
            move_steps.append(rot.as_matrix()[:3, :3])
        else:
            move_steps.append(dist_per_frame)
    ...
    for i in range(num_frames):
        for semantic, mesh in part_meshes.items():
            ...
            if is_rotate:
                R = move_steps[template_idx]
                if direction == -1:
                    R = R.T  # rotate backward
                mesh.rotate(R, center=move_centers[template_idx])
            else:
                move_dist = move_steps[template_idx] * direction
                mesh.translate(move_dist * move_axis, relative=True)
            vis.update_geometry(mesh)

...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    blade_concept = find_concept_by_template_name("blade")
    assert handle_concept["template"] in ["Multideck_Handle", "Enveloping_Handle"]

    if button_concept:
        move_center = [0, 0, 0]
        size = button_concept["parameters"]["size"]
        move_state_and_limits = []
        move_state_and_limits.append(np.array([0, size[1] * 2, 0]))
        move_state_and_limits.append(np.array([0, size[1] * 2, 0]))
        actionable_parts_info["template_names"] = [
            button_concept["template"],
            blade_concept["template"],
        ]
        actionable_parts_info["move_types"] = ["prismatic"] * 2
        actionable_parts_info["move_centers"] = [move_center] * 2
        actionable_parts_info["move_axises"] = [np.array([0, 1, 0])] * 2
        actionable_parts_info["move_state_and_limits"] = move_state_and_limits

    else:
        blade_rotation_center = blade_concept["parameters"]["position"]
        blade_rotation = blade_concept["parameters"]["rotation"][0]
        move_state_and_limits = []
        move_state_and_limits.append(np.array([0, 180, blade_rotation]))
        actionable_parts_info["template_names"] = [blade_concept["template"]]
        actionable_parts_info["move_types"] = ["revolute"]
        actionable_parts_info["move_centers"] = [blade_rotation_center]
        actionable_parts_info["move_axises"] = [np.array([1, 0, 0])]
        actionable_parts_info["move_state_and_limits"] = move_state_and_limits

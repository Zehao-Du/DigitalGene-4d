...
class Regular_Screen(ConceptTemplate):
    def __init__(
        self, size, offset, screen_rotation, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
...
class Cuboidal_Connector(ConceptTemplate):
    def __init__(
        ...
        # Record Parameters
        self.number_of_connector = [round(item) for item in number_of_connector]
        self.size = size
        self.offset = offset
        ...
        for i in range(round(number_of_connector[0])):
            back_mesh_position = [

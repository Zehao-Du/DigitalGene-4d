...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    # find lid_concept
    lid_concepts = find_concept_by_template_name("cylinder")
    body_concept = find_concept_by_template_name("body")[0]
    body_height = body_concept["parameters"]["position"][1]

    assert len(lid_concepts) <= 2
    lid_concept = None
    concept_idx = -1
    if len(lid_concepts) == 2:
        lid_1_concept = lid_concepts[0]
        lid_2_concept = lid_concepts[1]
        # select the one with higher height
        height_1 = lid_1_concept["parameters"]["position"][1]
        height_2 = lid_2_concept["parameters"]["position"][1]
        if height_1 > height_2:
            concept_idx = 1
            lid_concept = lid_1_concept
        else:
            concept_idx = 2
            lid_concept = lid_2_concept
    elif len(lid_concepts) == 1:
        concept_idx = 1
        lid_concept = lid_concepts[0]

    lid_height = (
        lid_concept["parameters"]["position"][1] if lid_concept is not None else 0
    )

    if (lid_concept is not None) and body_height < lid_height:
        move_center = [0, 0, 0]
        size = lid_concept["parameters"]["size"]
        move_state_and_limits = []
        move_state_and_limits.append(np.array([0, size[0], 0]))
        actionable_parts_info["template_names"] = [
            lid_concept["template"],
        ]
        actionable_parts_info["move_types"] = ["prismatic"]
        actionable_parts_info["move_centers"] = [move_center]
        actionable_parts_info["move_axises"] = [np.array([0, 1, 0])]
        actionable_parts_info["move_state_and_limits"] = move_state_and_limits
        actionable_parts_info["concept_idxes"] = [concept_idx]

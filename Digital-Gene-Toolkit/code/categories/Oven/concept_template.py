...
class Cuboidal_Body(ConceptTemplate):
    def __init__(self, size, thickness, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        bottom_mesh_position = [0, 0, -(size[2] - thickness[4]) / 2]
        self.bottom_mesh = Cuboid(
            size[1], size[0], thickness[4], position=bottom_mesh_position
        )
        ...
        top_mesh_position = [0, 0, thickness[4] / 2]
        top_mesh_rotation = [np.pi / 2, 0, 0]
        self.top_mesh = Rectangular_Ring(
            size[2] - thickness[4],
            size[0],
            size[1],
...
class Cuboidal_Door(ConceptTemplate):
    def __init__(self, size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.mesh = Cuboid(size[1], size[0], size[2], position=mesh_position)
...
class Sunken_Door(ConceptTemplate):
    def __init__(
        self, size, sunken_size, sunken_offset, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        super().__init__(position, rotation)
        ...
        bottom_mesh_position = [0, 0, -sunken_size[2] / 2 + size[2] / 2]
        self.bottom_mesh = Cuboid(
            size[1], size[0], size[2] - sunken_size[2], position=bottom_mesh_position
        )
        vertices_list.append(self.bottom_mesh.vertices)
        faces_list.append(self.bottom_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.bottom_mesh.vertices)

        top_mesh_position = [0, 0, (size[2] - sunken_size[2]) / 2 + size[2] / 2]
        top_mesh_rotation = [np.pi / 2, 0, 0]
        self.top_mesh = Rectangular_Ring(
            sunken_size[2],
            size[0],
...
class Cuboidal_Handle(ConceptTemplate):
    def __init__(self, size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.vertices = apply_transformation(
            self.vertices, position, rotation, rotation_order="ZXY"
        )
...
class Trifold_Handle(ConceptTemplate):
    def __init__(
        ...
        top_mesh_position = [0, mounting_seperation[0] / 2, mounting_size[2] / 2]
        ...
        bottom_mesh_position = [0, -mounting_seperation[0] / 2, mounting_size[2] / 2]
        self.bottom_mesh = Cuboid(
            mounting_size[1],
            mounting_size[0],
            mounting_size[2],
        ...
        vertical_mesh_position = [0, 0, mounting_size[2] + grip_size[2] / 2]
        self.vertical_mesh = Cuboid(
            grip_size[1], grip_size[0], grip_size[2], position=vertical_mesh_position
        )
...
class Trifold_Curve_Handle(ConceptTemplate):
    def __init__(
        ...
        curve_z_offset = mounting_size[2] - np.sqrt(
            curve_size[1] * curve_size[1]
            - (mounting_seperation[0] / 2) * (mounting_seperation[0] / 2)
        )
        vertical_mesh_position = [0, 0, curve_z_offset]
        vertical_mesh_rotation = [0, -np.pi / 2 + curve_exist_angle[0] / 2, np.pi / 2]
        self.vertical_mesh = Ring(
            curve_size[2],
            curve_size[0],
            curve_size[1],
            curve_exist_angle[0],
...
class Curve_Handle(ConceptTemplate):
    def __init__(
        self, curve_size, curve_exist_angle, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        curve_exist_angle = [x / 180 * np.pi for x in curve_exist_angle]
...
class Drawer_Like_Tray(ConceptTemplate):
    def __init__(self, outer_size, inner_size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        bottom_mesh_position = [0, -inner_size[1] / 2, 0]
        self.bottom_mesh = Cuboid(
        ...
        top_mesh_position = [0, (outer_size[1] - inner_size[1]) / 2, 0]
        self.top_mesh = Rectangular_Ring(
            inner_size[1],
            outer_size[0],
            outer_size[2],
...
class Controller_With_Button(ConceptTemplate):
    def __init__(
        ...
        mesh_position = [0, 0, bottom_size[3] / 2]
        self.mesh = Cuboid(
            bottom_size[1],
            bottom_size[0],
            bottom_size[2],
        ...
        for i in range(num_buttons[0]):
            mesh_position = [
                ...
                locals()["button_%d_offset" % (i + 1)][1] * np.cos(button_rotation)
                + locals()["button_%d_size" % (i + 1)][2] / 2 * np.sin(button_rotation),
                (bottom_size[2] + bottom_size[3]) / 2
                + locals()["button_%d_size" % (i + 1)][2] / 2 * np.cos(button_rotation)
                - locals()["button_%d_offset" % (i + 1)][1] * np.sin(button_rotation),
            ]
            mesh_rotation = [-button_rotation, 0, 0]
            self.mesh = Cuboid(
                locals()["button_%d_size" % (i + 1)][1],
                locals()["button_%d_size" % (i + 1)][0],
                locals()["button_%d_size" % (i + 1)][2],
                position=mesh_position,
...
class Top_With_Burner(ConceptTemplate):
    def __init__(
        ...
        mesh_position = [0, bottom_size[1] / 2, 0]
        self.mesh = Cuboid(
            bottom_size[1], bottom_size[0], bottom_size[2], position=mesh_position
        )
        vertices_list.append(self.mesh.vertices)
        faces_list.append(self.mesh.faces + total_num_vertices)
        total_num_vertices += len(self.mesh.vertices)
        ...
        for i in range(num_burners[0]):
            ...
            self.mesh = Rectangular_Ring(
                locals()["burner_%d_size" % (i + 1)][1],
                locals()["burner_%d_size" % (i + 1)][0],
                locals()["burner_%d_size" % (i + 1)][2],
                locals()["burner_%d_size" % (i + 1)][0]
                - locals()["burner_%d_thickness" % (i + 1)][0] * 2,
                locals()["burner_%d_size" % (i + 1)][2]
                - locals()["burner_%d_thickness" % (i + 1)][0] * 2,
                position=mesh_position,
            )
            ...
            center_mesh_position = [
                locals()["burner_%d_offset" % (i + 1)][0]
                + locals()["burner_%d_central_offset" % (i + 1)][0],
                bottom_size[1] + locals()["burner_%d_central_size" % (i + 1)][1] / 2,
                locals()["burner_%d_offset" % (i + 1)][1]
                + locals()["burner_%d_central_offset" % (i + 1)][1],
            ]
            self.center_mesh = Cylinder(
                locals()["burner_%d_central_size" % (i + 1)][1],
                locals()["burner_%d_central_size" % (i + 1)][0],
...
class Multilevel_Leg(ConceptTemplate):
    def __init__(
        ...
        if num_legs[0] == 1:
            mesh_position = [0, -front_legs_size[1] / 2, 0]
            self.mesh = Cuboid(
                front_legs_size[1],
                front_legs_size[0],
                front_legs_size[2],
            ...
            mesh_position = [legs_separation[0] / 2, -front_legs_size[1] / 2, 0]
            self.mesh = Cuboid(
                front_legs_size[1],
                front_legs_size[0],
                front_legs_size[2],
            ...
            mesh_position = [0, -rear_legs_size[1] / 2, -legs_separation[2] / 2]
            self.mesh = Cuboid(
                rear_legs_size[1],
                rear_legs_size[0],
                rear_legs_size[2],

...
class Double_Layer_Barrel(ConceptTemplate):
    def __init__(
        self, main_size, bottom_size, thickness, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        super().__init__(position, rotation)
        ...
        bottom_mesh_position = [0, -(main_size[2] + bottom_size[1]) / 2, 0]
        self.bottom_mesh = Ring(
            bottom_size[1],
            main_size[1],
            main_size[1] - thickness[0],
...
class Trifold_Clip(ConceptTemplate):
    def __init__(
        ...
        mesh_position = [0, -clip_root_size[1] / 2, clip_root_size[2] / 2]
        self.mesh = Cuboid(
            clip_root_size[1],
            clip_root_size[0],
            clip_root_size[2],
        ...
        self.middle_mesh = Cuboid(
            clip_vertical_size[1],
            clip_vertical_size[0],
            clip_vertical_size[2],
...
class Curved_Clip(ConceptTemplate):
    def __init__(
        ...
        mesh_rotation = [0, -np.pi / 2 + clip_curve_exist_angle[0] / 2, np.pi / 2]
        self.mesh = Ring(
            clip_curve_size[2],
            clip_curve_size[0],
            clip_curve_size[1],
            clip_curve_exist_angle[0],
            position=mesh_position,
...
class Cylindrical_Button(ConceptTemplate):
    def __init__(self, size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.mesh = Cylinder(size[2], size[0], size[1], position=mesh_position)
        vertices_list.append(self.mesh.vertices)
        faces_list.append(self.mesh.faces + total_num_vertices)
        total_num_vertices += len(self.mesh.vertices)
...
class Bistratal_Button(ConceptTemplate):
    def __init__(self, bottom_size, top_size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        mesh_position = [0, bottom_size[2] / 2, 0]
        self.mesh = Cylinder(
            bottom_size[2], bottom_size[0], bottom_size[1], position=mesh_position
        )
        vertices_list.append(self.mesh.vertices)
        faces_list.append(self.mesh.faces + total_num_vertices)
        total_num_vertices += len(self.mesh.vertices)

        top_mesh_position = [0, bottom_size[2] + top_size[2] / 2, 0]
...
class Cylindrical_Refill(ConceptTemplate):
    def __init__(
        ...
        bottom_mesh_position = [0, -size[1] / 2 - tip_height[0] / 2, 0]
        self.bottom_mesh = Cylinder(
            tip_height[0], tip_radius[0], position=bottom_mesh_position
        )
        vertices_list.append(self.bottom_mesh.vertices)
        faces_list.append(self.bottom_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.bottom_mesh.vertices)

        tip_mesh_position = [0, -size[1] / 2 - tip_height[0], 0]

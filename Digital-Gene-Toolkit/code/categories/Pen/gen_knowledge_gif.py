...
def generate_grasp_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, part_names=["button", "cap"]):
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Pen", manipulation_knowledge)
    )
    ...
    for move_axis, move_state_and_limit in zip(move_axises, move_state_and_limits):
        ...
        move_axis /= np.linalg.norm(move_axis)
        translate = np.array(move_axis * height_per_frame, dtype=np.float64)

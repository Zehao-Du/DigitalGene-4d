...
def generate_grasp_knowledge_gif(
    ...
    # only select the first handle + door + vessel concept to form a new conceptualization
    door_occur = 0
    for idx, template_name in enumerate(template_names):
        if "door" in template_name.lower():
            door_occur += 1
        if door_occur >= 2:
            break
    one_concept_cnt = idx  # the concept number of (handle + door + vessel) in concepts
    # rebuild the concept
    old_concepts = deepcopy(concepts)
    concepts = []
    for idx, concept_idx in enumerate(concept_idxes[:one_concept_cnt]):
        concepts.append(old_concepts[concept_idx])
    template_names = template_names[:one_concept_cnt]
    move_centers = move_centers[:one_concept_cnt]
    move_axises = move_axises[:one_concept_cnt]
    move_state_and_limits = move_state_and_limits[:one_concept_cnt]
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Refrigerator", manipulation_knowledge)
    )
...
def generate_affordance_knowledge_gif(
    ...
    for i in range(num_frames):
        template_name_cnt = {}  # indicate this is the i-th occurrence
        for idx, template_name in enumerate(template_names):
            if template_name not in template_name_cnt:
                template_name_cnt[template_name] = 0
            template_name_cnt[template_name] += 1
            template_idx = find_concept_idx_by_idx(
                concepts, template_name_cnt[template_name], template_name
            )
            mesh = part_meshes[f"{template_name}_{template_idx}"]
            direction = 1 if i < turn_times[idx] else -1
            R = rot_steps[idx]
            if direction == -1:
                R = R.T  # rotate backward
            mesh.rotate(R, center=move_centers[idx])
            vis.update_geometry(mesh)

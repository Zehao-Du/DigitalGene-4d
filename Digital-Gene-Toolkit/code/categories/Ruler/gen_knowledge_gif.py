...
def generate_grasp_knowledge_gif(
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Ruler", manipulation_knowledge)
    )
    gripper_mesh.scale(0.2, center=[0, 0, 0])
    gripper_mesh.scale(1.5, center=[0, 0, 0])
    ...
    for concept in concepts:
        if "body" in concept["template"]:
            body_concept = concept
            break
    assert body_concept is not None, "No body concept found in the list."
    body_rotations = body_concept["parameters"]["body_rotation"]

    images = []
    turn_times = []
    min_angle, max_angle = 0, 75  # degree
    for body_rotation in body_rotations:
        assert min_angle <= body_rotation <= max_angle, (
            f"Body rotation {body_rotation} is out of range [{min_angle}, {max_angle}]"
        )
        total_angle_range = (max_angle - body_rotation) + (max_angle - min_angle)
        turn_times.append(
            int(np.ceil(num_frames * (max_angle - body_rotation) / total_angle_range))
        )
    ...
    for i in range(3 * num_frames):
        if i < 2 * num_frames:
            ...
            for idx in range(len(turn_times)):
                direction = 1 if (i - 2 * num_frames) < turn_times[idx] else -1
                body_concept["parameters"]["body_rotation"][idx] += (
            ...
            grasp_pose, force_direction, gripper_width_ratio, _ = get_manip_knowledge(
                concepts, "Ruler", manipulation_knowledge
            )
            gripper_mesh = deepcopy(origin_gripper_mesh).transform(grasp_pose)
            vis.add_geometry(gripper_mesh)
            ctr = vis.get_view_control()
...
def generate_affordance_knowledge_gif(
    ...
    images = []
    turn_times = []
    min_angle, max_angle = 0, 90  # degree
    for body_rotation in body_rotations:
        assert min_angle <= body_rotation <= max_angle, (
            f"Body rotation {body_rotation} is out of range [{min_angle}, {max_angle}]"
        )

...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    # find cover_concept and magazine_concept
    cover_concept = find_concept_by_template_name("cover")
    magazine_concept = find_concept_by_template_name("magazine")
    ...
    cover_rotation_angle = cover_rotation[rotation_axis_idx]
    magazine_rotation_angle = magazine_concept["parameters"]["rotation"][0]
    ...
    assert cover_rotation_angle < 0, "cover rotation angle should be negative."

    axis_pos = np.array([0, 0, 0], dtype=np.float32)
    axis_pos += np.array(cover_concept["parameters"]["position"])

    move_state_and_limit = np.array([-70, -5, magazine_rotation_angle])

    actionable_parts_info["template_names"] = [
        cover_concept["template"],
        magazine_concept["template"],
    ]
    actionable_parts_info["move_types"] = ["revolute", "revolute"]
    actionable_parts_info["move_centers"] = [axis_pos, axis_pos]
    actionable_parts_info["move_axises"] = [rotation_axis, rotation_axis]
    actionable_parts_info["move_state_and_limits"] = [
        move_state_and_limit,
        move_state_and_limit,
    ]

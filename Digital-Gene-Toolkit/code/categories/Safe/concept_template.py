...
class Mutiple_Layer_Body(ConceptTemplate):
    def __init__(
        ...
        self.board_mesh = Cuboid(
            main_clapboard_size[0],
            size[0] - thickness[2] - thickness[3],
            main_clapboard_size[1],
        ...
        for i in range(num_of_sub_clapboards[0]):
            board_mesh_position = [
                (thickness[3] - thickness[2]) / 2
                + locals()["sub_clapboard_%d_offset" % (i + 1)][0],
                (thickness[1] - thickness[0]) / 2
                + locals()["sub_clapboard_%d_offset" % (i + 1)][1],
                thickness[4] / 2
                - (
                    size[2]
                    - thickness[4]
                    - locals()["sub_clapboard_%d_size" % (i + 1)][2]
                )
                / 2,
            ]
            self.board_mesh = Cuboid(
                locals()["sub_clapboard_%d_size" % (i + 1)][1],
                locals()["sub_clapboard_%d_size" % (i + 1)][0],
                locals()["sub_clapboard_%d_size" % (i + 1)][2],
                position=board_mesh_position,
            )
            vertices_list.append(self.board_mesh.vertices)
...
class Behind_Double_Layer_Door(ConceptTemplate):
    def __init__(
        ...
        mesh_position = [0, 0, main_size[2] / 2]
        self.mesh = Cuboid(
            main_size[1], main_size[0], main_size[2], position=mesh_position
        )
        vertices_list.append(self.mesh.vertices)
        faces_list.append(self.mesh.faces + total_num_vertices)
        total_num_vertices += len(self.mesh.vertices)

        behind_mesh_position = [behind_offset[0], behind_offset[1], -behind_size[2] / 2]
...
class Front_Double_Layer_Door(ConceptTemplate):
    def __init__(
        ...
        self.front_mesh = Cuboid(
            front_size[1], front_size[0], front_size[2], position=front_mesh_position
        )
...
class T_Shaped_Connecter(ConceptTemplate):
    def __init__(
        ...
        self.beside_mesh = Cuboid(
            lateral_cuboid_size[1],
            lateral_cuboid_size[0],
            lateral_cuboid_size[2],
...
class Trifold_Handle(ConceptTemplate):
    def __init__(
        ...
        top_mesh_position = [0, bottom_seperation[0] / 2, bottom_size[2] / 2]
        self.top_mesh = Cuboid(
            bottom_size[1], bottom_size[0], bottom_size[2], position=top_mesh_position
        )
        vertices_list.append(self.top_mesh.vertices)
        faces_list.append(self.top_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.top_mesh.vertices)

        bottom_mesh_position = [0, -bottom_seperation[0] / 2, bottom_size[2] / 2]
        self.bottom_mesh = Cuboid(
            bottom_size[1],
            bottom_size[0],
            bottom_size[2],
        ...
        vertical_mesh_position = [0, 0, bottom_size[2] + top_size[2] / 2]
        self.vertical_mesh = Cuboid(
            top_size[1], top_size[0], top_size[2], position=vertical_mesh_position
        )
...
class Claw_Handle(ConceptTemplate):
    def __init__(
        ...
        bottom_mesh_position = [0, 0, bottom_size[1] / 2]
        ...
        for i in range(num_forks[0]):
            ...
            rotate_length = bottom_size[0] + fork_size[0] / 2 * np.cos(
            ...
            mesh_position = [
                ...
                -fork_size[2] / 2
                + bottom_size[1]
                + fork_size[0] / 2 * np.sin(fork_tilt_rotation[0])
                - fork_offset[0],
            ]
            mesh_rotation = [0, -fork_tilt_rotation[0], rotation_tmp]
            self.mesh = Cuboid(
                fork_size[1],
                fork_size[0],
                fork_size[2],
...
class Round_Handle(ConceptTemplate):
    def __init__(
        ...
        circle_mesh_position = [
            ...
            bottom_size[1]
            - fork_size[2] / 2
            + fork_size[0] * np.sin(fork_tilt_rotation[0])
            - fork_offset[0],
        ]
        circle_mesh_rotation = [np.pi / 2, 0, 0]
        self.circle_mesh = Ring(
            circle_size[1],
            outer_radius,
            inner_radius,
...
class Cylindrical_Dial(ConceptTemplate):
    def __init__(self, bottom_size, top_size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        bottom_mesh_position = [0, 0, bottom_size[2] / 2]
        bottom_mesh_rotation = [-np.pi / 2, 0, 0]
        self.bottom_mesh = Cylinder(
            bottom_size[2],
            bottom_size[0],
        ...
        top_mesh_position = [0, 0, bottom_size[2] + top_size[1] / 2]
        top_mesh_rotation = [-np.pi / 2, 0, 0]
...
class Regular_Controller(ConceptTemplate):
    def __init__(self, size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        bottom_mesh_position = [0, 0, size[2] / 2]
        self.bottom_mesh = Cuboid(
            size[1], size[0], size[2], position=bottom_mesh_position
        )
        vertices_list.append(self.bottom_mesh.vertices)
        faces_list.append(self.bottom_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.bottom_mesh.vertices)

...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    door_concept = find_concept_by_template_name("door")
    handle_concept = find_concept_by_template_name("handle")
    dial_concept = find_concept_by_template_name("dial")
    controller_concept = find_concept_by_template_name("controller")
    connecter_concepts = find_concepts_by_template_name("connecter")
    ...
    if np.array_equal(door_rotation, [0, 0, 0]):
        ...
        if handle_concept is not None:
            handle_x_position = handle_concept["parameters"]["position"][0]
            if handle_x_position > 0:
                rotation_angle = -rotation_angle

        if connecter_concepts:
            connecter_concept = connecter_concepts[0]
            connecter_x_position = connecter_concept["parameters"]["position"][0]
            if connecter_x_position < 0:
                rotation_angle = -rotation_angle
    ...
    if door_concept["template"] in ["Cuboidal_Door", "Sunken_Door"]:
        size = door_concept["parameters"]["size"]
    elif door_concept["template"] in [
        "Behind_Double_Layer_Door",
        "Front_Double_Layer_Door",
    ]:
        size = door_concept["parameters"]["main_size"]
    else:
        raise ValueError(f"Unknown door template: {door_concept['template']}")
    if rotation_axis_idx == 0:
        if rotation_angle < 0: 
            rot_offset = np.array(
                [
                    0,
                    np.cos(rotation_angle_rad) * size[1] / 2,
                    np.sin(rotation_angle_rad) * size[1] / 2,
                ]
            )
        else:
            rot_offset = np.array(
        ...
        if rotation_angle >= 0: 
            rot_offset = np.array(
                [
                    np.cos(rotation_angle_rad) * size[0] / 2,
                    0,
                    -np.sin(rotation_angle_rad) * size[0] / 2,
                ]
            )
        else:
            rot_offset = np.array(
        ...
        raise ValueError("Rotation axis idx should be either '0'(x) or '1'(y).")
    axis_pos += rot_offset
    ...
    if handle_concept:
        actionable_parts_info["template_names"].append(handle_concept["template"])
    if dial_concept:
        actionable_parts_info["template_names"].append(dial_concept["template"])
    if controller_concept:
        actionable_parts_info["template_names"].append(controller_concept["template"])
    actionable_parts_info["move_types"] = ["revolute"] * concept_num
    actionable_parts_info["move_centers"] = [axis_pos] * concept_num
    actionable_parts_info["move_axises"] = [rotation_axis] * concept_num

    actionable_parts_info["template_names"].extend(
        [c["template"] for c in connecter_concepts]
    )
    actionable_parts_info["move_types"].extend(["revolute"] * len(connecter_concepts))
    actionable_parts_info["move_centers"].extend(
        [c["parameters"]["position"] for c in connecter_concepts]
    )
    actionable_parts_info["move_axises"].extend([[0, 1, 0] for _ in connecter_concepts])

    move_state_and_limits = np.zeros((concept_num + len(connecter_concepts), 3))

    if rotation_angle > 0:
        max_angle = max(90, rotation_angle)

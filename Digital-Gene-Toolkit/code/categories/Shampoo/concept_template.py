...
class Cylindrical_body(ConceptTemplate):
    def __init__(
        self, num_of_part, all_sizes, x_z_ratio, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        ...
        # Record Parameters
        self.num_of_part = [round(item) for item in num_of_part]
        self.all_sizes = [all_sizes[0:3]] + [
            [all_sizes[i], all_sizes[i + 1]] for i in range(3, len(all_sizes), 2)
        ]
        ...
        self.mesh_1 = Cylinder(
            height=height,
            top_radius=top_radius,
            bottom_radius=bottom_radius,
            top_radius_z=top_radius * x_z_ratio[0],
        ...
        for part_idx in range(1, self.num_of_part[0]):
            top_radius, bottom_radius, height = (
                self.all_sizes[part_idx][0],
                self.all_sizes[part_idx - 1][0],
                self.all_sizes[part_idx][1],
            )
            delta_height += height
            part_mesh_position = [0, delta_height - height / 2, 0]
            self.part_mesh = Cylinder(
                height=height,
                top_radius=top_radius,
                bottom_radius=bottom_radius,
                top_radius_z=top_radius * x_z_ratio[0],
...
class Toothpaste_body(ConceptTemplate):
    def __init__(
        self, radius, bottom_length, height, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        super().__init__(position, rotation)
...
class Regular_nozzle(ConceptTemplate):
    def __init__(
        ...
        self.parts_params = [
            [parts_params[i], parts_params[i + 1]]
            for i in range(0, len(parts_params), 2)
        ]
        ...
        delta_height = 0
        for part_idx in range(self.num_of_part[0]):
            delta_height += self.parts_params[part_idx][1]
            part_mesh_position = [
                0,
                delta_height - self.parts_params[part_idx][1] / 2,
                0,
            ]
            self.part_mesh = Cylinder(
                self.parts_params[part_idx][1],
                self.parts_params[part_idx][0],
                position=part_mesh_position,
            )
            vertices_list.append(self.part_mesh.vertices)
            faces_list.append(self.part_mesh.faces + total_num_vertices)
        ...
        nozzle_mesh_position = [
            ...
            delta_height
            + nozzle_offset[0]
            - nozzle_length[0] / 2 * np.sin(nozzle_rotation[0]),
            nozzle_length[0] / 2 * np.cos(nozzle_rotation[0])
            + self.parts_params[num_of_part[0] - 1][0],
        ]
        self.nozzle_mesh = Cuboid(
            nozzle_size[1],
            nozzle_size[0],
            nozzle_length[0],
            position=nozzle_mesh_position,
        )
        self.nozzle_mesh.vertices = apply_transformation(
            self.nozzle_mesh.vertices, [0, 0, 0], [0, nozzle_rotation[2], 0]
        )
        ...
        if num_of_nozzle == 2:
            other_nozzle_mesh_position = [
                0,
                delta_height
                + nozzle_offset[0]
                - nozzle_length[0] / 2 * np.sin(nozzle_rotation[0])
                - nozzle_length[1] / 2 * np.sin(nozzle_rotation[1]),
                nozzle_length[0] * np.cos(nozzle_rotation[0])
                + self.parts_params[num_of_part[0] - 1][0]
                + nozzle_length[1] * np.cos(nozzle_rotation[1]) / 2,
            ]
            self.other_nozzle_mesh = Cuboid(
                nozzle_size[1],
                nozzle_size[0],
                nozzle_length[0],
                position=other_nozzle_mesh_position,
            )
            self.other_nozzle_mesh.vertices = apply_transformation(
                self.other_nozzle_mesh.vertices, [0, 0, 0], [0, nozzle_rotation[2], 0]
            )
...
class Cylindrical_cap(ConceptTemplate):
    def __init__(
        self, outer_size, inner_size, x_z_ratio, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        super().__init__(position, rotation)
        ...
        self.nozzle_mesh = Ring(
            height=outer_size[2],
            outer_top_radius=outer_size[0],
        ...
        self.nozzle_mesh = Cylinder(
            height=outer_size[2] - inner_size[2],
            top_radius=outer_size[0],
            bottom_radius=outer_size[0],
            top_radius_z=outer_size[0] * x_z_ratio[0],
            bottom_radius_z=outer_size[0] * x_z_ratio[0],
            position=nozzle_mesh_position,
        )
        vertices_list.append(self.nozzle_mesh.vertices)
        faces_list.append(self.nozzle_mesh.faces + total_num_vertices)

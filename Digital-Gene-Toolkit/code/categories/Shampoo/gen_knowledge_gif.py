...
def generate_grasp_knowledge_gif(
    ...
    # some concepts may have 'parameter' instead of 'parameters'
    for concept in concepts:
        if "parameter" in concept:
            concept["parameters"] = concept.pop("parameter")
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Shampoo", manipulation_knowledge)
    )
    ...
    cap_concept = None
    for concept in concepts:
        if "cap" in concept["template"].lower():
            cap_concept = concept
            break
    for concept in concepts:
        if "nozzle" in concept["template"].lower():
            nozzle_concept = concept
            break
    assert (nozzle_concept is not None) or (cap_concept is not None), (
        "Nozzle or cap concepts must be provided for generating knowledge GIF."
    )
...
def generate_affordance_knowledge_gif(
    ...
    for move_axis, move_state_and_limit in zip(move_axises, move_state_and_limits):
        min_move, max_move, cur_move = move_state_and_limit
        total_move_range = (max_move - cur_move) + (max_move - min_move)
        turn_times.append(
            int(np.ceil(num_frames * (max_move - cur_move) / total_move_range))
        )
        move_per_frame = total_move_range / num_frames
        move_axis = move_axis / np.linalg.norm(move_axis)
        if is_rotate:
            rot = Rot.from_rotvec(move_per_frame * move_axis, degrees=True)
            move_steps.append(rot.as_matrix()[:3, :3])
        else:
            move_steps.append(move_per_frame * move_axis) 
    ...
    for i in range(num_frames):
        for semantic, mesh in part_meshes.items():
            ...
            if is_rotate:
                ...
                move_vector = direction * move_steps[template_idx]
                mesh.translate(move_vector, relative=True)
            vis.update_geometry(mesh)

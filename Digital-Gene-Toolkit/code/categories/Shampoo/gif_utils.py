...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    Args:
        conceptualization (list): A list of dictionaries, where each dictionary describes a
                         component and its parameter (including position, rotation)
    ...
    if cap_concept is not None:
        move_center = [0, 0, 0]
        outer_size = cap_concept["parameters"]["outer_size"]
        move_state_and_limits = []
        move_state_and_limits.append(np.array([0, outer_size[2] * 2, 0]))
        actionable_parts_info["template_names"] = [
            cap_concept["template"],
        ]
        ...
        nozzle_concept = find_concept_by_template_name("nozzle")
        assert nozzle_concept is not None, (
            "No nozzle concept found in the conceptualization."
        )
        move_center = nozzle_concept["parameters"]["position"]
        move_state_and_limits = []
        move_state_and_limits.append(np.array([0, 360, 0]))
        actionable_parts_info["template_names"] = [
            nozzle_concept["template"],
        ]

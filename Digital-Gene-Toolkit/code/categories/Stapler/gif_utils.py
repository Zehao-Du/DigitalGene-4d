...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    move_state_and_limit = np.array([-70, -5, -abs(cover_rotation_angle)])

    actionable_parts_info["template_names"] = [
        cover_concept["template"],
    ]
    num_concept = 1
    if magazine_concept is not None:
        actionable_parts_info["template_names"].append(magazine_concept["template"])
        num_concept += 1

    actionable_parts_info["move_types"] = ["revolute"] * num_concept
    actionable_parts_info["move_centers"] = [axis_pos] * num_concept
    actionable_parts_info["move_axises"] = [rotation_axis] * num_concept

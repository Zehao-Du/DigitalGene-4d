...
def generate_grasp_knowledge_gif(
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "StorageFurniture", manipulation_knowledge)
    )
...
def generate_affordance_knowledge_gif(
    ...
    if door_concept is not None:
        ...
        door_rotations = [
            doors_params[i * 12 + 8] for i in range(int(number_of_door[0]))
        ]
        door_offsets = [
            doors_params[i * 12 + 9 : i * 12 + 12]
            for i in range(int(number_of_door[0]))
        ]
        door_sizes = [
            doors_params[i * 12 : i * 12 + 3] for i in range(int(number_of_door[0]))
        ]
    ...
    if drawer_concept is not None:
        ...
        drawer_size = [
            drawers_params[i * 20 : i * 20 + 3] for i in range(number_of_drawer[0])
        ]
        drawer_offset = [
        ...
        for idx in range(number_of_drawer[0]):
            cur_dist = drawer_offset[idx][2]  # z-axis direction
    ...
    for frame in range(num_frames):
        ...
        if door_concept is not None:
            ...
            direction = np.zeros((int(number_of_door[0]),), dtype=np.int32)
            for idx in range(len(turn_times["door"])):
                direction[idx] = 1 if frame < turn_times["door"][idx] else -1
            new_door_rotations = [
                doors_params[i * 12 + 8] + direction[i] * move_per_frames["door"][i]
                for i in range(int(number_of_door[0]))
            ]
        ...
        if drawer_concept is not None:
            ...
            for i in range(number_of_drawer[0]):
                ...
                updated_drawers_params[i * 20 + 17 : i * 20 + 20] = new_drawer_offsets[
                    i
                ]

...
def assemble_concepts_instance(concepts):
    ...
    for c in concepts:
        ...
        try:
            ...
            if module_name in ["Regular_drawer", "Storagefurniture_body"]:
                if module_name == "Regular_drawer":
                ...
                elif module_name == "Storagefurniture_body":
                    per_part_vertices = [0]
                    per_part_faces = [0]
                    part2instance = []
                    part_cnt = 0

                    has_lid = component.has_lid
                    body_mesh_number = 4
                    lid_mesh_number = 1 if has_lid[0] == 1 else 0
                    WHOLE_layer_mesh_number = component.WHOLE_number_of_layer[0]
                    TOTAL_EACH_layer_mesh_number = sum(component.EACH_number_of_layer)
                    TOTAL_mesh_list = [
                        body_mesh_number,
                        lid_mesh_number,
                    ...
                    EACH_number_of_layer = component.EACH_number_of_layer
                    number_of_additional_layers = component.number_of_additional_layers
                    ...
                    for space_idx in range(WHOLE_layer_mesh_number + 1):
                        for mesh_idx in range(EACH_number_of_layer[space_idx]):
                            per_part_vertices += [8]
                            per_part_faces += [12]
                            part2instance += [part_cnt]
                        part_cnt += 1
                    for additional_idx in range(number_of_additional_layers):
                        per_part_vertices += [8]
                        per_part_faces += [12]
                        part2instance += [part_cnt]
                        part_cnt += 1
                ...
                if module_name == "Enclosed_leg":
                    number_of_additional_legs = component.number_of_additional_legs
                    num_instance = number_of_additional_legs + 4
                elif module_name == "Regular_door":
                    number_of_door = component.number_of_door
                    num_instance = int(number_of_door[0] * 2)
                elif module_name == "Regular_front_panel":
                    number_of_frontPanel = component.number_of_frontPanel
                    num_instance = int(number_of_frontPanel[0])
                else:
                    num_instance = 1

...
class Standard_Base(ConceptTemplate):
    def __init__(
        ...
        self.back_mesh = Cuboid(size[1], size[0], size[2], position=back_mesh_position)
...
class Frame_Base(ConceptTemplate):
    def __init__(
        ...
        self.back_mesh = Rectangular_Ring(
            size[2],
            size[0],
            size[1],
            inner_size[0],
...
class Parallel_Base(ConceptTemplate):
    def __init__(self, size, sub_offset, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        left_mesh_position = [-size[0] / 2, 0, -size[4] / 2]
        self.left_mesh = Cuboid(size[2], size[0], size[4], position=left_mesh_position)
        ...
        right_mesh_position = [size[1] / 2, sub_offset[0], -size[4] / 2]
        self.right_mesh = Cuboid(
            size[3], size[1], size[4], position=right_mesh_position
        )
...
class Round_Switch(ConceptTemplate):
    def __init__(
        ...
        # Record Parameters
        self.number_of_switch = [round(item) for item in number_of_switch]
        self.size = size
        ...
        for i in range(round(number_of_switch[0])):
            base_mesh_position = [
...
class Cuboidal_Plug(ConceptTemplate):
    def __init__(
        ...
        # Record Parameters
        self.column_of_contact = [round(item) for item in column_of_contact]
        self.row_of_contact = [round(item) for item in row_of_contact]
        self.size = size
        ...
        for j in range(round(row_of_contact[0])):
            for i in range(round(column_of_contact[0])):
                mesh_position = (
                    [
                        (interval[0] + size[0]) * i,
                        (interval[1] + size[1]) * j,
...
class Standard_Plug(ConceptTemplate):
    def __init__(
        self, size, sub_offset, plug_rotation, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        ...
        mesh_position = [
            -sub_offset[0] * np.cos(plug_rotation[0])
            + sub_offset[1] * np.sin(-plug_rotation[0]),
            sub_offset[1] * np.cos(-plug_rotation[0])
            - sub_offset[0] * np.sin(plug_rotation[0]),
            -size[2] / 2,
        ]

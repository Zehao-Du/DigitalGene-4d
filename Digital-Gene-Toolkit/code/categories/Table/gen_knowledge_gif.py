...
def generate_grasp_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, part_names=["door", "drawer"]):
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "Table", manipulation_knowledge)
    )
    gripper_mesh.scale(0.2, center=[0, 0, 0])
    gripper_mesh.scale(0.4, center=[0, 0, 0])
    ...
    for i in range(gripper_frames_num):
        if i < gripper_frames_num // 4:
            gripper_params.append([cur_gripper_param] * 2)
    ...
    # -1 -> 0 -> 1
    per_frame_delta = 2 / gripper_frames_num
    cur_gripper_param = -1
    for i in range(gripper_frames_num):
        cur_gripper_param += per_frame_delta
        gripper_params[i] += [cur_gripper_param] * 2

    door_concept, drawer_concept = None, None
    for concept in concepts:
        if concept["template"].lower() == "regular_door":
            door_concept = concept
            break
    for concept in concepts:
        if concept["template"].lower() == "regular_drawer":
            drawer_concept = concept
            break
    assert drawer_concept is not None or door_concept is not None, (
        "No drawer or door concept found in the list."
    )
...
def generate_affordance_knowledge_gif(
    ...
    images = []
    turn_times = {"door": [], "drawer": []}
    move_per_frames = {"door": [], "drawer": []}
    door_concept, drawer_concept = None, None
    for concept in concepts:
        if concept["template"].lower() == "regular_door":
            door_concept = concept
    ...
    if door_concept is not None:
        # don't transform to rad
        doors_params = door_concept["parameters"]["doors_params"]
        number_of_door = door_concept["parameters"]["number_of_door"]
        door_rotations = [doors_params[i * 13 + 9] for i in range(number_of_door[0])]
        door_offsets = [
            doors_params[i * 13 + 10 : i * 13 + 13] for i in range(number_of_door[0])
        ]
        door_sizes = [
            doors_params[i * 13 : i * 13 + 3] for i in range(number_of_door[0])
        ]
        for door_rotation in door_rotations:
            if door_rotation > 0:
                min_angle, max_angle = 0, max(90, door_rotation)  # degree
            else:
                min_angle, max_angle = min(-90, door_rotation), 0
            assert door_rotation >= min_angle and door_rotation <= max_angle, (
                "door rotation out of range, please check door_concept"
            )
            total_angle_range = (max_angle - door_rotation) + (max_angle - min_angle)
            turn_times["door"].append(
                int(
                    np.ceil(
                        num_frames * (max_angle - door_rotation) / total_angle_range
                    )
                )
            )
            angle_per_frame = total_angle_range / num_frames
            move_per_frames["door"].append(angle_per_frame)

        door_init_axis_pos = deepcopy(door_offsets)
        for i in range(number_of_door[0]):
            theta_angle_rad = np.deg2rad(door_rotations[i])
            door_x = door_sizes[i][0] / 2
            if door_rotations[i] < 0:
                door_x = -door_x
            # only modify x and z coordinates
            door_init_axis_pos[i] = door_offsets[i] + np.array(
                [
                    door_x * (np.cos(theta_angle_rad)),
                    0,
                    door_x * (-np.sin(theta_angle_rad)),
                ]
            )
    if drawer_concept is not None:
        drawers_params = drawer_concept["parameters"]["drawers_params"]
        number_of_drawer = drawer_concept["parameters"]["number_of_drawer"]
        drawer_size = [
            drawers_params[i * 21 : i * 21 + 3] for i in range(number_of_drawer[0])
        ]
        drawer_offset = [
        ...
        for idx in range(number_of_drawer[0]):
            cur_dist = drawer_offset[idx][2]  # z-axis
            min_dist = cur_dist - drawer_size[idx][2] / 2
            max_dist = cur_dist + drawer_size[idx][2] / 2
            total_dist_range = max_dist - min_dist
            turn_times["drawer"].append(
                int(np.ceil(num_frames * (max_dist - cur_dist) / total_dist_range))
            )
            dist_per_frame = total_dist_range / num_frames
            move_per_frames["drawer"].append(dist_per_frame)

    for frame in range(num_frames):
        vis.clear_geometries()
        # Direction Control: First Half Forward,Second Half Backward
        if door_concept is not None:
            updated_doors_params = deepcopy(np.array(doors_params))
            direction = np.zeros((number_of_door[0],), dtype=np.int32)
            for idx in range(len(turn_times["door"])):
                direction[idx] = 1 if frame < turn_times["door"][idx] else -1
            new_door_rotations = [
                doors_params[i * 13 + 9] + direction[i] * move_per_frames["door"][i]
                for i in range(number_of_door[0])
            ]
            new_door_offsets = np.array(
                [
            ...
            if debug:
                debug_move_centers = door_init_axis_pos
                debug_move_axises = [np.array([0, 1, 0], dtype=np.float64)] * len(
                    door_init_axis_pos
                )
                debug_draw_move_axis(vis, debug_move_centers, debug_move_axises)

            for i in range(number_of_door[0]):
                theta_angle_rad = np.deg2rad(new_door_rotations[i])
                door_x = door_sizes[i][0] / 2
                if door_rotations[i] < 0:
                    door_x = -door_x
                new_door_offsets[i] = door_init_axis_pos[i] + np.array(
                    [
                        door_x * (-np.cos(theta_angle_rad)),
                        0,
                        door_x * (np.sin(theta_angle_rad)),
            ...
            for i in range(number_of_door[0]):
                idx = 13 * i + 9
                updated_doors_params[idx] = new_door_rotations[i]
                idx = 13 * i + 10
                updated_doors_params[idx : idx + 3] = new_door_offsets[i]

            doors_params[:] = updated_doors_params

        if drawer_concept is not None:
            updated_drawers_params = deepcopy(np.array(drawers_params))
            direction = np.zeros((number_of_drawer[0],), dtype=np.int32)
            for idx in range(len(turn_times["drawer"])):
                direction[idx] = 1 if frame < turn_times["drawer"][idx] else -1
            new_drawer_offsets = np.array(
                [
                    drawers_params[i * 21 + 18 : i * 21 + 21]
                    for i in range(number_of_drawer[0])
                ]
            ...
            for i in range(number_of_drawer[0]):
                new_drawer_offsets[i][2] += direction[i] * move_per_frames["drawer"][i]
                updated_drawers_params[i * 21 + 18 : i * 21 + 21] = new_drawer_offsets[
                    i
                ]

            drawers_params[:] = updated_drawers_params

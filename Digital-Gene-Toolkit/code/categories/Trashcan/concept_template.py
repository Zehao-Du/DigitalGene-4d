...
class Cylindrical_Body(ConceptTemplate):
    def __init__(self, outer_size, inner_size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        middle_radius = (
            outer_size[0] * (1 - inner_size[2] / outer_size[2])
            + outer_size[1] * inner_size[2] / outer_size[2]
        )
        bottom_height = outer_size[2] - inner_size[2]
        bottom_mesh_position = [0, -inner_size[2] / 2, 0]
        self.bottom_mesh = Cylinder(
            bottom_height, middle_radius, outer_size[1], position=bottom_mesh_position
        )
...
class Prismatic_Body(ConceptTemplate):
    def __init__(
        ...
        mesh_position = [0, -(height[1] - height[0]) / 2, 0]
        self.mesh = Rectangular_Ring(
            height[0],
            top_size[0],
            top_size[1],
            top_size[0] - thickness[0] * 2,
            top_size[1] - thickness[1] * 2,
            outer_bottom_length=bottom_size[0],
            outer_bottom_width=bottom_size[1],
            inner_bottom_length=bottom_size[0] - thickness[0] * 2,
            inner_bottom_width=bottom_size[1] - thickness[1] * 2,
        ...
        middle_x = (top_size[0] - thickness[0] * 2) * thickness[2] / height[0] + (
            bottom_size[0] - thickness[0] * 2
        ) * (height[0] - thickness[2]) / height[0]
        middle_z = (top_size[1] - thickness[1] * 2) * thickness[2] / height[0] + (
            bottom_size[1] - thickness[1] * 2
        ) * (height[0] - thickness[2]) / height[0]
        middle_offset_x = top_offset[0] * thickness[2] / height[0]
        middle_offset_z = top_offset[1] * thickness[2] / height[0]
        bottom_mesh_position = [0, -(height[1] - thickness[2]) / 2, 0]
        self.bottom_mesh = Cuboid(
            thickness[2],
            middle_x,
            middle_z,
            bottom_length=bottom_size[0] - thickness[0] * 2,
            bottom_width=bottom_size[1] - thickness[1] * 2,
...
class Separated_Cylindrical_Body(ConceptTemplate):
    def __init__(
        ...
        clapboard_mesh_position = [
            0,
            clapboard_size[1] / 2
            - (outer_size[2] / 2 - (outer_size[2] - inner_size[2])),
            0,
        ]
        self.clapboard_mesh = Cuboid(
            clapboard_size[1],
            clapboard_size[0],
            clapboard_size[2],
...
class Double_Layer_Cuboidal_Cover(ConceptTemplate):
    def __init__(
        self, top_size, bottom_size, top_offset, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        # Process rotation param
        rotation = [x / 180 * np.pi for x in rotation]
        super().__init__(position, rotation)
        ...
        self.top_mesh = Cuboid(
            top_size[1], top_size[0], top_size[2], position=top_mesh_position
        )
        vertices_list.append(self.top_mesh.vertices)
        faces_list.append(self.top_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.top_mesh.vertices)
...
class Cylindrical_Hollow_Cover(ConceptTemplate):
    def __init__(self, size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.mesh = Ring(size[2], size[0], size[1], position=mesh_position)
        vertices_list.append(self.mesh.vertices)
        faces_list.append(self.mesh.faces + total_num_vertices)
        total_num_vertices += len(self.mesh.vertices)
...
class Holed_Cylindrical_Cover(ConceptTemplate):
    def __init__(
        ...
        self.top_mesh = Cylinder(
            height[0] - height[1], radius[0], position=top_mesh_position
        )
        vertices_list.append(self.top_mesh.vertices)
        faces_list.append(self.top_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.top_mesh.vertices)

        middle_mesh_position = [0, height[2] + height[1] / 2, 0]
        self.middle_mesh = Ring(
            height[1], radius[0], radius[1], position=middle_mesh_position
        )
...
class Holed_Cuboidal_Cover(ConceptTemplate):
    def __init__(
        ...
        # front part
        if has_hole[0] == 0:
            mesh_position = [0, inner_size[1] / 2, (outer_size[2] + inner_size[2]) / 4]
            self.mesh = Cuboid(
                inner_size[1],
                outer_size[0],
                (outer_size[2] - inner_size[2]) / 2,
                position=mesh_position,
            )
            vertices_list.append(self.mesh.vertices)
            faces_list.append(self.mesh.faces + total_num_vertices)
            ...
            self.mesh_1 = Cuboid(
                inner_size[1],
                (outer_size[0] - front_behind_hole_size[0]) / 2,
                (outer_size[2] - inner_size[2]) / 2,
            ...
            self.mesh_3 = Cuboid(
                inner_size[1] - front_behind_hole_size[1],
                front_behind_hole_size[0],
                (outer_size[2] - inner_size[2]) / 2,
        ...
        # left part
        if has_hole[2] == 0:
            mesh_position = [-(outer_size[0] + inner_size[0]) / 4, inner_size[1] / 2, 0]
            self.mesh = Cuboid(
                inner_size[1],
            ...
            self.mesh_1 = Cuboid(
                inner_size[1],
                (outer_size[0] - inner_size[0]) / 2,
                (inner_size[2] - left_right_hole_size[0]) / 2,
                position=mesh_1_position,
            )
            vertices_list.append(self.mesh_1.vertices)
            faces_list.append(self.mesh_1.faces + total_num_vertices)
            ...
            self.mesh_3 = Cuboid(
                inner_size[1] - left_right_hole_size[1],
                (outer_size[2] - inner_size[2]) / 2,
                left_right_hole_size[0],
                position=mesh_3_position,
            )
            vertices_list.append(self.mesh_3.vertices)
            faces_list.append(self.mesh_3.faces + total_num_vertices)
...
class Standard_Wheel(ConceptTemplate):
    def __init__(self, size, seperation, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        left_mesh_position = [-seperation[0], 0, 0]
        left_mesh_rotation = [0, 0, np.pi / 2]
        self.left_mesh = Cylinder(
            size[1], size[0], position=left_mesh_position, rotation=left_mesh_rotation
        )
        ...
        right_mesh_position = [seperation[0], 0, 0]
        right_mesh_rotation = [0, 0, np.pi / 2]
        self.right_mesh = Cylinder(
            size[1], size[0], position=right_mesh_position, rotation=right_mesh_rotation
        )
...
class Cuboidal_Shell(ConceptTemplate):
    def __init__(self, outer_size, inner_size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.mesh = Rectangular_Ring(
            outer_size[1], outer_size[0], outer_size[2], inner_size[0], inner_size[1]
        )
        vertices_list.append(self.mesh.vertices)
        faces_list.append(self.mesh.faces + total_num_vertices)
        total_num_vertices += len(self.mesh.vertices)

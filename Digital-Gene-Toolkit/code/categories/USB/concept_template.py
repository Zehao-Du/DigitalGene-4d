...
class Regular_Body(ConceptTemplate):
    def __init__(
        self, size, has_back_part, has_side_part, position=[0, 0, 0], rotation=[0, 0, 0]
    ):
        ...
        self.main_mesh = Cuboid(size[1], size[0], size[2], position=main_mesh_position)
...
class Simplied_Connector(ConceptTemplate):
    def __init__(self, size, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.main_mesh = Cuboid(size[1], size[0], size[2], position=back_mesh_position)
        vertices_list.append(self.main_mesh.vertices)
        faces_list.append(self.main_mesh.faces + total_num_vertices)
        total_num_vertices += len(self.main_mesh.vertices)
...
class Regular_Connector(ConceptTemplate):
    def __init__(self, size, thickness, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        self.main_mesh = Rectangular_Ring(
            size[2],
            size[0],
            size[1],
            size[0] - thickness[0] * 2,
            size[1] / 2 - thickness[0],
...
class Regular_Cap(ConceptTemplate):
    def __init__(
        ...
        self.ring_mesh = Rectangular_Ring(
            inner_size[2],
            size[0],
            size[1],
            inner_size[0],
        ...
        self.end_mesh = Cuboid(
            size[1], size[0], size[2] - inner_size[2], position=end_mesh_position
        )
...
class SquareEnded_Cap(ConceptTemplate):
    def __init__(
        ...
        middle_position = [
            back_position[0],
            (radius + size[1] / 2) * np.cos(inclination[0])
            - size[2] * np.sin(inclination[0]) / 2
            + back_position[1],
            (radius + size[1] / 2) * np.sin(inclination[0])
            + size[2] * np.cos(inclination[0]) / 2
            + back_position[2],
        ]

        topCuboid_mesh_position = [
            middle_position[0] * np.cos(cap_rotation[0])
            + middle_position[2] * np.sin(cap_rotation[0]),
            middle_position[1],
            middle_position[0] * np.sin(cap_rotation[0])
            + middle_position[2] * np.cos(cap_rotation[0])
            + shaft_offset[0],
        ]
        topCuboid_mesh_rotation = [inclination[0], cap_rotation[0], 0]
        self.topCuboid_mesh = Cuboid(
            size[1],
            size[0],
            size[2],
        ...
        self.c_mesh.vertices = apply_transformation(
            self.c_mesh.vertices, c_mesh_position, [0, cap_rotation[0], 0]
        )
        ...
        if has_shaft[0]:
            ...
            self.bottomShaft_mesh = Cylinder(
                shaft_size[1],
                shaft_size[0],
                shaft_size[0],
...
class RoundEnded_Cap(ConceptTemplate):
    def __init__(
        ...
        middle_position = [
            back_position[0],
            (radius + size[1] / 2) * np.cos(inclination[0])
            - (size[2] - size[0] / 2) * np.sin(inclination[0]) / 2
            + back_position[1],
            (radius + size[1] / 2) * np.sin(inclination[0])
            + (size[2] - size[0] / 2) * np.cos(inclination[0]) / 2
            + back_position[2],
        ]
        end_position = [
            middle_position[0],
            (
                size[1] * np.cos(inclination[0])
                + size[0] * np.sin(inclination[0])
                + proximal_interval[0]
            )
            / 2,
            -(-size[1] * np.sin(inclination[0]) + size[0] * np.cos(inclination[0])) / 2
            - shaft_offset[0],
        ]
        ...
        topEnd_mesh_position = [
            end_position[0] * np.cos(cap_rotation[0])
            + end_position[2] * np.sin(cap_rotation[0]),
            end_position[1],
            end_position[0] * np.sin(cap_rotation[0])
            + end_position[2] * np.cos(cap_rotation[0])
            + shaft_offset[0],
        ]
        topEnd_mesh_rotation = [inclination[0], cap_rotation[0], 0]
        self.topEnd_mesh = Cylinder(
            size[1],
            size[0] / 2,
            size[0] / 2,
            is_half=True,

...
def generate_grasp_knowledge_gif(
    ...
    if not find_crucial_parts(template_names, part_names=["cap"]):
    ...
    grasp_pose, force_direction, gripper_width_ratio, manip_params_size = (
        get_manip_knowledge(concepts, "USB", manipulation_knowledge)
    )
    gripper_mesh.scale(0.2, center=[0, 0, 0])
    gripper_mesh.scale(2, center=[0, 0, 0])
    ...
    for i in range(3 * num_frames):
        if i < 2 * num_frames:
            ...
            for semantic, mesh in part_meshes.items():
                if "cap" in semantic.lower():
                    semantic = "cap"
                    assert find_text_in_list(semantic, template_names) != -1, (
                        f"semantic {semantic} not in template_names {template_names}"
                    )
                ...
                if is_rotate:
                    ...
                    move_dist = move_steps[template_idx]
                    mesh.translate(move_dist * move_axis, relative=True)
                    if semantic.lower() == "cap":
                        gripper_mesh.translate(move_dist * move_axis, relative=True)
                        vis.update_geometry(gripper_mesh)
                vis.update_geometry(mesh)
            vis.poll_events()
            vis.update_renderer()

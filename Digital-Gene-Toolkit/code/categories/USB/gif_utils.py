...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    cap_concept = find_concepts_by_template_name("cap")
    body_concept = find_concepts_by_template_name("body")
    if cap_concept["template"] in ["RoundEnded_Cap", "SquareEnded_Cap"]:
        cap_rotation_center = [0, 0, cap_concept["parameters"]["shaft_offset"][0]]

        move_state_and_limits = []
        move_state_and_limits.append(np.array([0, 360, 0]))

        actionable_parts_info["template_names"] = [cap_concept["template"]]
        actionable_parts_info["move_types"] = ["revolute"]
        actionable_parts_info["move_centers"] = [cap_rotation_center]
        actionable_parts_info["move_axises"] = [np.array([0, 1, 0])]
        actionable_parts_info["move_state_and_limits"] = move_state_and_limits
    else:  # regular_cap
        cap_position_z = cap_concept["parameters"]["position"][2]
        body_position_z = body_concept["parameters"]["position"][2]
        ...
        move_state_and_limits.append(
            np.array(
                [
                    body_position_z,
                    cap_position_z + (cap_position_z - body_position_z),
                    cap_position_z,
                ]
            )
        )
        ...
        actionable_parts_info["move_centers"] = [[0, 0, 0]]
        actionable_parts_info["move_axises"] = [np.array([0, 0, 1])]
        actionable_parts_info["move_state_and_limits"] = move_state_and_limits

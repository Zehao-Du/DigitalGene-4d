...
def analyze_actionable_parts_from_conceptualization(conceptualization):
    ...
    if door_concept["template"] == "Roller_Door":
        size = [0, 0, 0]  # [door_concept["parameters"]["circle_size"][0]] * 3
    else:
        size = door_concept["parameters"]["size"]
    # rotation can be divided into four cases: y-axis left/right, x-axis up/down, z-axis left/right
    if rotation_axis_idx == 0:  # x-axis up
        assert rotation_angle <= 0, (
            "Rotation angle for x-axis should be negative (downward)."
        )
        rot_offset = np.array(
            [
                0,
                np.sin(rotation_angle_rad) * size[2] / 2,
                -np.cos(rotation_angle_rad) * size[2] / 2,
            ]
        )
    elif rotation_axis_idx == 1:  # y-axis left
        rot_offset = np.array(
            [
                0,
                np.sin(rotation_angle_rad) * size[2] / 2,
        ...
        if rotation_angle > 0:  # z-axis left
            rot_offset = np.array(
                [

    ...
    def __init__(
        ...
        for configuration in window_configurations:
            configuration["window_size"]["glass_offset"][0] = (
                configuration["window_size"]["glass_offset"][0]
                * configuration["asymmetryFlag"]
            )
            configuration["position"][1] = (
                configuration["position"][1] + outside_frame_inner_outer_offset[1]
            )
        ...
        for configuration in window_configurations:
            ...
            for i in range(3):
                glass_mesh_position.append(
                    ...
                    + configuration["window_size"]["glass_offset"][i]
                )
    ...
    def __init__(
        ...
        for i in range(num_of_handle[0]):
            ...
            self.handle_mesh = Cuboid(
                size[1], size[0], size[2], position=handle_mesh_position
            )
...
class Arched_Handle(ConceptTemplate):
    def __init__(
        ...
        central_angle = (
            np.arcsin((seperation[0] / 2 + bottom_size[1]) / outer_size[0]) * 2
        )
...
class LShaped_Handle(ConceptTemplate):
    def __init__(
        ...
        for i in range(num_of_handle[0]):
            ...
            self.top_mesh = Cuboid(
                size_top[1], size_top[0], size_top[2], position=top_mesh_position
            )
            vertices_list.append(self.top_mesh.vertices)
            faces_list.append(self.top_mesh.faces + total_num_vertices)
            total_num_vertices += len(self.top_mesh.vertices)

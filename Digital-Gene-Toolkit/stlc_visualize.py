...
geometry_template_module = importlib.import_module("code.geometry_template")
...
def draw_arrow(scale):
    ...
    cylinder = geometry_template_module.Cylinder(
        scale,
        scale * 0.15,
    ...
    cone = geometry_template_module.Cone(
        scale * 0.3,
        scale * 0.4,
...
def render_conceptualization_to_mesh(data):
    ...
    for c in data["conceptualization"]:
        module = getattr(concept_template_module, c["template"])
...
if __name__ == "__main__":
    pkl_path = f"./assets/conceptualizations/stlc/{args.category}.pkl"
    ...
    for data in data_list:
        ...
        # Get object model
        if has_object_model:
            source_path = f"{object_model_path}/{data['id']}.obj"
            ...
            source_mesh_vertices = source_mesh_vertices + np.array(
                [-mesh_scale * 1.5, 0, 0]
            )
            ...
            source_mesh = o3d.geometry.TriangleMesh(
                source_mesh_vertices, source_mesh_faces
            )
        ...
        if has_object_model:
            o3d.visualization.draw_geometries(
                [conceptualization_rendering, arrow_mesh, source_mesh]
            )
        else:

...
class Object:
    def __init__(
        ...
        Object is a container of StructureNode instances. For simple objects,
        passing position and rotation creates one contained StructureNode.
        '''
        self.Nodes = []
        self.Edges = list(edges) if edges is not None else []
        self.semantic = semantic
        self.prompt = prompt if prompt is not None else object_prompt if object_prompt is not None else semantic
        self.Object_Prompt = self.prompt
        self.pose_6d = pose_6d

        if nodes is not None:
            if isinstance(nodes, StructureNode):
                nodes = [nodes]
    ...
    def _node_attr(self, name):
        values = [getattr(node, name) for node in self.Nodes]
        if len(values) == 1:
            return values[0]
        return values
    ...
    @property
    def position(self):
        return self._node_attr("position")

    @property
    def rotation(self):
        return self._node_attr("rotation")

    @property
    def rotation_order(self):
        return self._node_attr("rotation_order")

    @property
    def Node_Position(self):
        return self._node_attr("Node_Position")

    @property
    def Node_Semantic(self):
        return self._node_attr("Node_Semantic")

    @property
    def Node_Affordance(self):
        return self._node_attr("Node_Affordance")

    @property
    def Node_Face(self):
        return self._node_attr("Node_Face")

    @property
    def Node_Axis(self):
        return self._node_attr("Node_Axis")

    @property
    def Refrence_Anchor(self):
        return self._node_attr("Refrence_Anchor")
...
class Map_4d:
    def __init__(self, objects=None):
        ...
        A Map_4d stores all Object instances in the scene and exposes flattened
        node/edge lists for downstream construction, segmentation, and pose
        estimation modules.
        '''
        self.Objects = []
        ...
        self.object_node_slices = {}
        self.Subgraph_Prompts = []

        for obj in objects or []:
            self.add_object(obj)

    def add_object(self, obj):
        object_idx = len(self.Objects)
        node_start = len(self.Nodes)
        nodes = list(getattr(obj, "Nodes", []))
        edges = list(getattr(obj, "Edges", []))

        self.Objects.append(obj)
        self.Nodes.extend(nodes)

        for edge in edges:
            edge.update_node_idx(node_start)
            self.Edges.append(edge)

        node_indices = list(range(node_start, node_start + len(nodes)))
        self.object_node_slices[object_idx] = node_indices
        self.Subgraph_Prompts.append(
            {
                "text_prompt": self._object_prompt(obj),
                "node_indices": node_indices,
            }
        )
        return obj

    def _object_prompt(self, obj):
        for attr in ("prompt", "Object_Prompt", "semantic"):
            prompt = getattr(obj, attr, None)
